/************************************************************************
File pds_heat_ls_intf_opencl - routines for interactions with linear solver
                          implemented for opencl

Contains definitions of routines:

  pdr_create_assemble_stiff_mat_opencl - to create element stiffness matrices
...
// Master switch: CPU versus GPU versus PHI - also controlled by compilation options !!!!!
//#define OPENCL_CPU
//#define OPENCL_GPU
//#define OPENCL_PHI

// Important switches for different variants of the algorithm:

// SWITCH 1: float versus double (MUST BE COMPATIBLE WITH KERNEL SWITCH!!!!)
// data type for integration
//#define SCALAR float - rather for testing only, in practice we use double
#define SCALAR double

// SWITCH 2: one_el_one_thread strategy versus one_el_one_workgroup strategy
#define ONE_EL_ONE_THREAD
//#define ONE_EL_ONE_WORKGROUP - another procedure will be designe d for this version

// SWITCH 3: Jacobian on GPU or CPU (CPU version obsolete - but may require two kernel 
...
// SWITCH 4: size for  work-group - can be further changed according to hardware platform
#ifdef OPENCL_CPU
#define WORK_GROUP_SIZE 8  // depends on assumptions in OpenCL compiler for CPUs !!!!!
#elif defined OPENCL_GPU
#define WORK_GROUP_SIZE 64 // recommended for both AMD and NVIDIA
#elif defined OPENCL_PHI
#define WORK_GROUP_SIZE 16 // depends on assumptions in OpenCL compiler for Xeon PHI !!!!!
#endif

// SWITCH 5: problem type
//#define LAPLACE
//#define TEST_SCALAR
#define HEAT
...
#ifdef HEAT
  #include "../pdd_heat/include/pdh_heat.h"
...
/*------------------------------------------------------------
 pdr_create_assemble_stiff_mat_opencl - to create element stiffness matrices
                                 and assemble them to the global SM
------------------------------------------------------------*/
int pdr_create_assemble_stiff_mat_opencl(
  int Problem_id, 
  int Level_id, 
  int Comp_type,         /* in: indicator for the scope of computations: */
  //extern const int PDC_NO_COMP  ; /* do not compute stiff matrix and rhs vector */
...
/*kbw*/
  printf("In pdr_create_assemble_stiff_mat_opencl: Problem_id %d, Level_id %d\n",
	 Problem_id, Level_id);
...
/*kbw
  printf("In pdr_create_assemble_stiff_mat before loop over entities\n");
...
/*kbw
 printf("In pdr_create_assemble_stiff_mat before calling pdr_comp_stiff_mat for faces\n");
...
// ****************************************************************
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//#pragma omp critical (pdr_comp_stiff_mat) - pdr_comp_stiff_mat HAVE TO BE THREAD SAFE!!!
      {
...
#pragma omp critical(assembling) // COLORING SHOULD BE USED !!!!!!!
      {
...
/*kbw
      printf("In pdr_create_assemble_stiff_mat in thread %d\n", my_thread_id);
/*kew*/
...
/*kbw
 ...
 printf("In pdr_create_assemble_stiff_mat before calling pdr_comp_stiff_mat for elems\n");
...
/*------------------------------------------------------------
 pdr_create_assemble_stiff_mat_opencl_elem - to create element stiffness matrices
                                 and assemble them to the global SM
!!! the version below is adapted to heat problem module - but is kept as close
!!! as possible to the generic version (to create, mey be, a single, parametrized
!!! version for all problem types in the future)
------------------------------------------------------------*/
int pdr_create_assemble_stiff_mat_opencl_elem(
  int Problem_id, 
  int Level_id, 
...
//kew*/
    ...
    int ngauss;            /* number of gauss points (150 for p=505) */
    double xg[450];   	 /* coordinates of gauss points in 3D */
    double wg[150];       /* gauss weights */
    ...
    // for each gauss point its coordinates and weight are sent for the reference element
    ...
    one_el_pde_coeff_size = num_dofs+1; // element size + DOFs from previous time step;
...
/*kew*/
    ...
    int global_memory_req_in = NR_EXEC_PARAMS; // check if correct !!! size of array with execution parameters
    int global_memory_req_one_el_in = 0;
    int global_memory_req_out = 0; // 
...
/*kbw*/
  printf("\nGlobal memory requirements (before computing the number of elements considered):\n");
  printf("\tglobal and the reference element %d, for each element %d\n",
...
// GLOBAL GPU MEMORY CONSIDERATIONS FOR SETTING THE NUMBER OF ELEMENTS PER KERNEL
    ...
    // CHECK 1 - limit by the size of OpenCL device memory
...
// how many workgroups will be enough to make CPU/GPU cores (comp_units) busy?
#ifdef OPENCL_CPU
#define NR_WORK_GROUPS_PER_COMP_UNIT  2 // ???? hyperthreading
#elif defined OPENCL_GPU
#define NR_WORK_GROUPS_PER_COMP_UNIT  8 // ????
#elif defined OPENCL_PHI
#define NR_WORK_GROUPS_PER_COMP_UNIT  4 // ???? hyperthreading and lack of out of order execution
#endif

    // usually it is good to maximize the number of work_groups and threads
	...
	nr_elems_per_work_group = (temp / work_group_size) * work_group_size;
...
/*kbw
    printf("\nInitial settings: nr_elems_per_kernel %d =  nr_elems_per_work_group %d *  nr_work_groups %d\n",
	   nr_elems_per_kernel, nr_elems_per_work_group, nr_work_groups);
//kew*/
...
// element data are in two el_data_in buffers:
// el_data_in_geo with size el_data_in_geo_bytes - for geometry data and
// el_data_in_coeff with size el_data_in_coeff_bytes - for weak form coeff data
...
// OpenCL Best Practices Guide (NVIDIA, 14.02.2011)

    // size of input buffer for element data
    const size_t el_data_in_geo_bytes =
      global_memory_req_one_el_in * nr_elems_per_kernel * sizeof(SCALAR);

    cl_mem cmPinnedBufInGeo = NULL; // input buffer mapped to host input buffer
    cmPinnedBufInGeo = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
				   el_data_in_geo_bytes, NULL, NULL);


    cl_mem cmDevBufInGeo = NULL; // device input buffer allocated in card's memory
    cmDevBufInGeo = clCreateBuffer(context, CL_MEM_READ_ONLY,
				el_data_in_geo_bytes, NULL, NULL);
  
    SCALAR* el_data_in_geo = NULL; // host input buffer (standard array)
    el_data_in_geo = (SCALAR*)clEnqueueMapBuffer(command_queue, cmPinnedBufInGeo, CL_TRUE,
    						 CL_MAP_WRITE, 0, el_data_in_geo_bytes, 0,
    						 NULL, NULL, NULL);

    retval |= clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *) &cmDevBufInGeo);

    // size of input buffer for weak form element data
    const size_t el_data_in_coeff_bytes =
      global_memory_req_one_el_in * nr_elems_per_kernel * sizeof(SCALAR);

    cl_mem cmPinnedBufInCoeff = NULL; // input buffer mapped to host input buffer
    cmPinnedBufInCoeff = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
				   el_data_in_coeff_bytes, NULL, NULL);


    cl_mem cmDevBufInCoeff = NULL; // device input buffer allocated in card's memory
    cmDevBufInCoeff = clCreateBuffer(context, CL_MEM_READ_ONLY,
				el_data_in_coeff_bytes, NULL, NULL);
  
    SCALAR* el_data_in_coeff = NULL; // host input buffer (standard array)
    el_data_in_coeff = (SCALAR*)clEnqueueMapBuffer(command_queue, cmPinnedBufInCoeff, CL_TRUE,
    						 CL_MAP_WRITE, 0, el_data_in_coeff_bytes, 0,
    						 NULL, NULL, NULL);

    retval |= clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *) &cmDevBufInCoeff);
    ...
    retval |= clSetKernelArg(kernel, 5, sizeof(cl_mem), (void *) &cmDevBufOut);
...
/*kbw*/
    ...
    printf("\tel_data_in_geo bytes %.3lf[MB], el_data_in_coeff bytes %.3lf[MB], el_data_out bytes %.3lf [MB]\n",
	   (double)el_data_in_geo_bytes*1.0e-6, (double)el_data_in_coeff_bytes*1.0e-6, 
	   (double)el_data_out_bytes*1.0e-6);
//kew*/

#ifdef TUNING
    fprintf(resf,"\tel_data_in_geo [MB];%.3lf;el_data_in_coeff [MB];%.3lf;el_data_out [MB];%.3lf;",
	   (double)el_data_in_geo_bytes*1.0e-6, (double)el_data_in_coeff_bytes*1.0e-6, 
	    (double)el_data_out_bytes*1.0e-6);
#endif
...
// FILL MEMORY OBJECTS WITH DATA AND SEND INPUT DATA TO GPU MEMORY
	...
	(pdt_heat_problem *)pdr_get_problem_structure(Problem_id);
      ...
      double implicitness_parameter = problem->time.alpha;
      
      execution_parameters_host[2] = 1000*implicitness_parameter;
      execution_parameters_host[3] = 1000000*delta_t;
#endif
...
#define MAX_SIZE_ARRAY_GAUSS 600
	// 600 - maximal value of arrGaussSize (for p=505)
...
//kew*/
	...
	free(shape_fun_host);
      ...
      int ielem;
      int packed_bytes_geo = 0;
      int packed_bytes_coeff = 0;
      
      memset(el_data_in_geo, 0, el_data_in_geo_bytes);
      memset(el_data_in_coeff, 0, el_data_in_coeff_bytes);
	...
	for(i=0;i<num_geo_dofs;i++){
	  el_data_in_geo[position_geo] = geo_dofs[3*i];
	  el_data_in_geo[position_geo+1] = geo_dofs[3*i+1];
	  el_data_in_geo[position_geo+2] = geo_dofs[3*i+2];
	  position_geo += 3;
	}
	
	packed_bytes_geo += num_geo_dofs*3*sizeof(SCALAR);
	...
	for(ki=0;ki<ngauss;ki++)
	  {
	    ...
	    pdr_el_coeff(Problem_id, el_id, el_mate, el_hsize, pdeg, NULL,
			 NULL, NULL, NULL, NULL,
	    ...
	    el_data_in_coeff[position_coeff+ki] = sval[0];
...
/*kbw
	    ...
	    int i,ueq;
		  ...
		  printf("Problem_id %d, el_id %d, el_mate %d, el_hsize %lf, pdeg %d\n",
		   Problem_id, el_id, el_mate, el_hsize, pdeg);
	...
	packed_bytes_coeff += ngauss*sizeof(SCALAR);
...
/*kew*/
	...
	el_data_in_coeff[position_coeff+0] = axx[0];
	el_data_in_coeff[position_coeff+1] = axy[0];
	el_data_in_coeff[position_coeff+2] = axz[0];
	el_data_in_coeff[position_coeff+3] = ayx[0];
	el_data_in_coeff[position_coeff+4] = ayy[0];
	el_data_in_coeff[position_coeff+5] = ayz[0];
	el_data_in_coeff[position_coeff+6] = azx[0];
	el_data_in_coeff[position_coeff+7] = azy[0];
	el_data_in_coeff[position_coeff+8] = azz[0];
	
	el_data_in_coeff[position_coeff+9] = bx[0];
	el_data_in_coeff[position_coeff+10] = by[0];
	el_data_in_coeff[position_coeff+11] = bz[0];
	
	el_data_in_coeff[position_coeff+12] = tx[0];
	el_data_in_coeff[position_coeff+13] = ty[0];
	el_data_in_coeff[position_coeff+14] = tz[0];
	
	el_data_in_coeff[position_coeff+15] = cval[0];
	
	el_data_in_coeff[position_coeff+16] = qx[0];
	el_data_in_coeff[position_coeff+17] = qy[0];
	el_data_in_coeff[position_coeff+18] = qz[0];
	
	el_data_in_coeff[position_coeff+19] = sval[0];
	position_coeff += 20;
	packed_bytes_coeff += 20*sizeof(SCALAR);
	...
	int sol_vec_id = 3; // solution from the previous time step
...
/*kbw
  printf("m_k %lf, norm_u %lf, h_k %lf\n",
	 m_k, norm_u, h_k);
/*kew*/

	el_data_in_coeff[position_coeff] = h_k;
	position_coeff++;

	int idofs;
	for (idofs = 0; idofs < num_shap; idofs++) {
	  el_data_in_coeff[position_coeff+idofs] = sol_dofs_n[idofs];
	}
	position_coeff+=num_shap;
	...
	for(ki=0;ki<ngauss;ki++) {
	  ...
	  el_data_in_coeff[position_coeff] = velocity[0];
	  el_data_in_coeff[position_coeff+1] = velocity[1];
	  el_data_in_coeff[position_coeff+2] = velocity[2];
	  position_coeff+=3;
	  
	  el_data_in_coeff[position_coeff] = thermal_conductivity;
	  position_coeff++;
	  
	  el_data_in_coeff[position_coeff] = dens_spec_heat;
	  position_coeff++;
	  
	}

	packed_bytes_coeff += (1 + num_shap + ngauss*(3+1+1))*sizeof(SCALAR);
	...
	// removed - we always perform Jacobian calculations on accelerators
	// see old versions for comparison
      ...
      assert(packed_bytes_geo == position_geo*sizeof(SCALAR));
      assert(packed_bytes_coeff == position_coeff*sizeof(SCALAR));
...
/*kbw*/
	...
	printf("el_data_in_geo[%d] = %f\n", i, el_data_in_geo[i]);
	//printf("el_data_in_geo[%d] = %lf\n", i, el_data_in_geo[i]);
	
      }
      for(i=position_geo-5;i<position_geo;i++){
	printf("el_data_in_geo[%d] = %f\n", i, el_data_in_geo[i]);
	//printf("el_data_in_geo[%d] = %lf\n", i, el_data_in_geo[i]);
	...
	printf("el_data_in_coeff[%d] = %f\n", i, el_data_in_coeff[i]);
	//printf("el_data_in_coeff[%d] = %lf\n", i, el_data_in_coeff[i]);
...
//kew*/
      ...
      assert(packed_bytes_geo<=el_data_in_geo_bytes);
      // writing input data to global GPU memory
      clEnqueueWriteBuffer(command_queue, cmDevBufInGeo, CL_FALSE, 0,
      			   el_data_in_geo_bytes, el_data_in_geo, 0, NULL, NULL);
      
      assert(packed_bytes_coeff<=el_data_in_coeff_bytes);
      // writing input data to global GPU memory
      clEnqueueWriteBuffer(command_queue, cmDevBufInCoeff, CL_FALSE, 0,
      			   el_data_in_coeff_bytes, el_data_in_coeff, 0, NULL, NULL);

#ifdef TIME_TEST
      clFinish(command_queue);
      t_end = time_clock();
      printf("\nEXECUTION TIME: sending el_data_in to GPU memory %lf (speed %lf [GB/s])\n", 
	     t_end-t_begin, (el_data_in_geo_bytes+el_data_in_coeff_bytes)*1.0e-9/(t_end-t_begin));
#ifdef TUNING
...
#define TESTING_GPU_TRANSFER
	...
	free(  shape_fun_host);
      ...
      clEnqueueReadBuffer(command_queue, cmDevBufInGeo, CL_TRUE, 0,
			  el_data_in_geo_bytes, el_data_in_geo, 0, NULL, NULL);
	...
	printf("el_data_in_geo[%d] = %lf\n", i, el_data_in_geo[i]);
      ...
      clEnqueueReadBuffer(command_queue, cmDevBufInCoeff, CL_TRUE, 0,
			  el_data_in_coeff_bytes, el_data_in_coeff, 0, NULL, NULL);
...
#pragma omp critical(assembling)
    ...
    clEnqueueUnmapMemObject(command_queue, cmPinnedBufInGeo, el_data_in_geo, 0, NULL, NULL);
    clReleaseMemObject(cmPinnedBufInGeo);
    clReleaseMemObject(cmDevBufInGeo);
    clEnqueueUnmapMemObject(command_queue, cmPinnedBufInCoeff, el_data_in_coeff, 0, NULL, NULL);
    clReleaseMemObject(cmPinnedBufInCoeff);
    clReleaseMemObject(cmDevBufInCoeff);
