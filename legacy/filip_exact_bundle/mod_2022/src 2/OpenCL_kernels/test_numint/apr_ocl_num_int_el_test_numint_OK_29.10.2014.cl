...
#define LOOP_INVARIANT_CODE_MOTION
...
SCALAR nr_global_access=0.0;
  ...
  for(ielem = 0; ielem < nr_elems_per_thread; ielem++){
    ...
    nr_access += 4;
      ...
      // geometrical shape functions are stored in tempx (if necessary)
      //SCALAR temp1 = ((one-daux-faux)*(one-eaux))*half;
      //SCALAR temp2 = daux*(one-eaux)*half;
      //SCALAR temp3 = faux*(one-eaux)*half;
      //SCALAR temp4 = (one-daux-faux)*(one+eaux)*half;
      //SCALAR temp5 = daux*(one+eaux)*half;
      //SCALAR temp6 = faux*(one+eaux)*half;
      //SCALAR temp7 = zero;
      //SCALAR temp8 = zero;
      //SCALAR temp9 = zero;
 ...
 //nr_oper += 13; // after optimization?
      ...
      { // block to indicate the scope of jac_data
        ...
        // derivatives of geometrical shape functions are stored in shape_fun_workspace
	shape_fun_workspace[thread_id*3*num_geo_dofs+0] = -(one-eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+1] =  (one-eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+2] =  zero;
	shape_fun_workspace[thread_id*3*num_geo_dofs+3] = -(one+eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+4] =  (one+eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+5] =  zero;
	shape_fun_workspace[thread_id*3*num_geo_dofs+6] = -(one-eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+7] =  zero;
	shape_fun_workspace[thread_id*3*num_geo_dofs+8] =  (one-eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+9] = -(one+eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+10] =  zero;
	shape_fun_workspace[thread_id*3*num_geo_dofs+11] =  (one+eaux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+12] = -(one-daux-faux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+13] = -daux*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+14] = -faux*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+15] =  (one-daux-faux)*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+16] =  daux*half;
	shape_fun_workspace[thread_id*3*num_geo_dofs+17] =  faux*half;

#ifdef COUNT_OPER
    nr_access += 18;
#endif

#else // if not USE_SHAPE_FUN_WORKSPACE_FOR_JAC_DATA
...
#endif // end if not USE_SHAPE_FUN_WORKSPACE_FOR_JAC_DATA
...
#ifdef USE_SHAPE_FUN_WORKSPACE_FOR_JAC_DATA
	  jac_1 = shape_fun_workspace[thread_id*3*num_geo_dofs+i];
	  jac_2 = shape_fun_workspace[thread_id*3*num_geo_dofs+num_geo_dofs+i];
	  jac_3 = shape_fun_workspace[thread_id*3*num_geo_dofs+2*num_geo_dofs+i];
 ...
 // total: 13+5+18*num_geo_dofs+15+36+15*num_shap = 177+90 = 267 (for prisms)
...
#ifdef LOOP_INVARIANT_CODE_MOTION
	...
	temp7 = coeff30*shape_fun_workspace[thread_id*3*num_shap+3*idof] +
        ...
        #else // if not SHAPE_FUN_REF_IN_CACHE
	...
	temp7 = coeff30*shape_fun_workspace[thread_id*4*num_shap+4*idof+1] +
        ...
        #endif  // end if not SHAPE_FUN_REF_IN_CACHE
	
#ifdef COUNT_OPER
	nr_access += 16; // constant cache and shared memory accesses
#endif
...
#endif // end if LOOP_INVARIANT_CODE_MOTION
...
#ifdef COUNT_OPER
    nr_access += 4; // constant cache and shared memory accesses
#endif
 ...
 nr_oper += 9; // after optimization?
   ...
   #else // if not LOOP_INVARIANT_CODE_MOTION
 ...
 shp_fun_v
  ...
  #endif // end if not LOOP_INVARIANT_CODE_MOTION
...
#else //NOT USE_SHAPE_FUN_WORKSPACE
  fun_u_derx * fun_v_derx +
  fun_u_dery * fun_v_dery +
  fun_u_derz * fun_v_derz
...
#endif // end if stiff_mat in shared
...
#ifdef COUNT_OPER
 // TOTAL FOR FULL MATRICES AND VECTORS:
 // (13+5+18*num_geo_dofs+15+36+15*num_shap+37*num_shap*num_shap+9*num_shap)*num_gauss =
 //          = (177+90+37*36+9*6)*6 = (267+1332+54)*6 = 9918 (for prisms) (with shape_fun_workspace)
 // (13+5+18*num_geo_dofs+15+36+15*num_shap+15*num_shap*num_shap+37*num_shap*num_shap+9*num_shap)*num_gauss=
