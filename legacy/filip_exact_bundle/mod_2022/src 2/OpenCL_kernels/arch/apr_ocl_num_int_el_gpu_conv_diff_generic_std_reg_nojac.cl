  ...
  for(ielem = 0; ielem < nr_elems_per_work_group; ielem++){
    ...
    //for(ipart = 0; ipart < nr_parts_of_stiff_mat; ipart++){
      ...
      int offset = (group_id*nr_elems_per_work_group + ielem)
                     * (NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC*num_gauss + J_AND_DETJ_SIZE*num_gauss);
#else
      int offset = (group_id*nr_elems_per_work_group + ielem)
...
#ifdef LAPLACE
      if(thread_id < NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC*num_gauss){
#else
      if(thread_id < NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC){
#endif

	pde_coeff_workspace[thread_id] = el_data_in[offset + thread_id];
//	if(ielem==0&&group_id==1)
//	{
//		stiff_mat_out[thread_id] = el_data_in[offset + thread_id];
//	}
      ...
      // SCALAR PROBLEMS
#ifdef LAPLACE
      offset += NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC*num_gauss;
      //int off=NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC*num_gauss;
#else
      offset += NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC;
      //int off=NR_PDE_COEFF_MAT + NR_PDE_COEFF_VEC;
#endif
      // VECTOR PROBLEMS     
...
//	if(ielem==0&&group_id==1)
//	{
//		stiff_mat_out[off+thread_id] = el_data_in[offset + thread_id];
//	}
...
// 7 or 10 operations - laplace
	  ...
	  if(thread_id < num_shap){
	    ...
	    load_vec_entry += (
			       pde_coeff_workspace[3+igauss] * shp_fun_v
			       //shp_fun_v 
			       //) * el_data_jac[igauss*J_AND_DETJ_SIZE + 9];
			       ) * vol;
#endif
