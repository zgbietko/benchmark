...
/*kew*/
  ...
  //second kernel for computing jac data

  int kernel_version_alg = device_struct.num_int_kernel_version/10 ; 
  
  int kernel_index_jac=TMC_OCL_KERNEL_NUM_INT_SEP_JAC;

  cl_kernel kernel_jac = tmr_ocl_select_kernel(platform_index, device_index, kernel_index_jac);
...
/*kew*/
  ...
  if(context == NULL || command_queue == NULL || kernel == NULL || kernel_jac == NULL){
...
//kew*/
    ...
    //FK zmiana dla aligmentu
    //ref_el_shape_fun_size = 4*WORK_GROUP_SIZE*ngauss;
...
#define NR_EXEC_PARAMS 16  // size of array with execution parameters
    ...
    int global_memory_req_in_jac = NR_EXEC_PARAMS; // size of array with execution parameters
    int global_memory_req_one_el_in_jac = 0;
    ...
    global_memory_req_in_jac += ref_el_quadr_dat_size;
    global_memory_req_one_el_in_jac += one_el_quadr_dat_size;

    // 2. GEO_DOFS - geo_dofs for JAC, nothing for NOJAC 
    global_memory_req_one_el_in_jac += one_el_geo_dat_size;
    ...
    int global_memory_req = global_memory_req_in + global_memory_req_in_jac + global_memory_req_out;
    int global_memory_req_one_el = global_memory_req_one_el_in + global_memory_req_one_el_in_jac + global_memory_req_one_el_out;
...
// GLOBAL GPU MEMORY CONSIDERATIONS FOR SETTING THE NUMBER OF ELEMENTS PER KERNEL
    ...
    int global_limiter;
    if(global_memory_req_one_el_in > global_memory_req_one_el_out && global_memory_req_one_el_in > global_memory_req_one_el_in_jac) {
      global_limiter = global_memory_req_one_el_in;
    }
    else if(global_memory_req_one_el_out > global_memory_req_one_el_in && global_memory_req_one_el_out > global_memory_req_one_el_in_jac)
    {
      global_limiter = global_memory_req_one_el_out;
    }
    else
    {
    	global_limiter = global_memory_req_one_el_in_jac;
    }
...
#elif defined OPENCL_GPU
#define NR_WORK_GROUPS_PER_COMP_UNIT  8 // TESLA according to Nvidia Profiler
#elif defined OPENCL_PHI
#define NR_WORK_GROUPS_PER_COMP_UNIT  1 // ????
#endif
    ...
    //8 to za dużo tutaj
    int nr_work_groups_jac = 0.25 * max_num_comp_units;
    int nr_elems_per_work_group;
    int nr_elems_per_wg_jac;

    printf("Tutaj worg=%d, nr_elems_per_kernel=%d\n",nr_work_groups,nr_elems_per_kernel);
    printf("Tutaj worg_jac=%d\n",nr_work_groups_jac);
    ...
    int work_group_size_jac = WG_SIZE_JAC;
    int work_group_size = WORK_GROUP_SIZE;

    //wymuszenie zmiany nr wg jac

    if(nr_elems_per_kernel > nr_elems_per_color + nr_work_groups_jac*work_group_size_jac) {
	
	nr_elems_per_wg_jac =
	  ceil(((double)nr_elems_per_color) / nr_work_groups_jac / work_group_size_jac) * work_group_size_jac;
	...
	int temp = nr_elems_per_kernel / nr_work_groups_jac;
	temp = temp / work_group_size_jac;
	nr_elems_per_wg_jac = temp * work_group_size_jac;
    ...
    int nr_elems_per_kernel_jac = nr_work_groups_jac * nr_elems_per_wg_jac;


/*kbw*/
    printf("\nInitial settings: nr_elems_per_work_group %d = nr_elems_per_kernel %d / nr_work_groups %d\n",
	   nr_elems_per_work_group, nr_elems_per_kernel, nr_work_groups);
    printf("\nInitial settings: nr_elems_per_wg_jac %d = nr_elems_per_kernel_jac %d / nr_work_groups %d\n",
    	   nr_elems_per_wg_jac, nr_elems_per_kernel_jac, nr_work_groups_jac);
//kew*/

#define MIN_NR_ELEMS_PER_WORK_GROUP_JAC WG_SIZE_JAC // ???
...
#define MIN_NR_ELEMS_PER_WORK_GROUP 1 // ??? 
    ...
    if(nr_elems_per_wg_jac < MIN_NR_ELEMS_PER_WORK_GROUP_JAC) {

		printf("Too low number of elements per workgroup_jac %d \n", nr_elems_per_wg_jac );
		printf("Check parameters of execution. Exiting!!!\n");
		exit(-1);
...
//kew*/
    ...
    int nr_threads_jac = nr_work_groups_jac * work_group_size_jac;
    float nr_elems_per_thread_jac = 0;
    float nr_elems_per_thread = 0;


    nr_elems_per_thread_jac = nr_elems_per_wg_jac / work_group_size_jac;
...
/*kbw*/
    ...
    printf("\tnr_elems_per_kernel_jac %d, nr_elems_per_work_group_jac %d, nr_elems_per_thread %.1f\n",
    	   nr_elems_per_kernel_jac,  nr_elems_per_wg_jac, nr_elems_per_thread_jac);
    printf("\tnr_work_groups_jac %d, work_group_size_jac %d, nr_threads_jac %d\n",
    		   nr_work_groups_jac, work_group_size_jac, nr_threads_jac );
...
//kew*/
      ...
      if(nr_elems_per_wg_jac != nr_elems_per_thread_jac*work_group_size_jac){
	printf("nr_elems_per_work_group_jac %d != %d nr_elems_per_thread_jac*work_group_size_jac\n",
			nr_elems_per_wg_jac, nr_elems_per_thread_jac*work_group_size_jac );
	printf("Exiting!\n");
	exit(-1);
      }
...
// functions and their derivatives for reference element)
    ...
    retval |= clSetKernelArg(kernel_jac, 0, sizeof(cl_mem), (void *) &execution_parameters_dev);
...
//kew*/
    ...
    if(gauss_dat_dev_bytes == 0){
      ...
      retval |= clSetKernelArg(kernel_jac, 1, sizeof(cl_mem), (void *) &gauss_dat_dev);
...
//kew*/
    ...
    if(shape_fun_dev_bytes == 0){
      ...
      retval |= clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *) NULL);

    }
    else{
//tu mu coś nie pasuje (constatnt)
      shape_fun_dev = clCreateBuffer(context, CL_MEM_READ_ONLY, shape_fun_dev_bytes, NULL, NULL);
      retval |= clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *) &shape_fun_dev);
...
// the page-locked memory for faster transfers
    ...
    const size_t el_data_in_jac_bytes =
          global_memory_req_one_el_in_jac * nr_elems_per_kernel_jac * sizeof(SCALAR);
...
// For CPU better result is when we don't use pinned buffer
    ...
    retval |= clSetKernelArgSVMPointer(kernel, 3, el_data_in);
		...
		cl_mem cmPinnedBufInJac = NULL; // input buffer mapped to host input buffer
		cmPinnedBufInJac = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
							   el_data_in_jac_bytes, NULL, NULL);
		...
		cl_mem cmDevBufInJac = NULL; // device input buffer allocated in card's memory
		cmDevBufInJac = clCreateBuffer(context, CL_MEM_READ_ONLY,
							el_data_in_jac_bytes, NULL, NULL);



		SCALAR* el_data_in_pde = NULL; // host input buffer (standard array)
		SCALAR* el_data_in_jac = NULL;

	#ifdef PINNED
		el_data_in_pde = (SCALAR*)clEnqueueMapBuffer(command_queue, cmPinnedBufIn, CL_TRUE,
								 CL_MAP_WRITE, 0, el_data_in_bytes, 0,
								 NULL, NULL, NULL);
		el_data_in_jac = (SCALAR*)clEnqueueMapBuffer(command_queue, cmPinnedBufInJac, CL_TRUE,
								 CL_MAP_WRITE, 0, el_data_in_jac_bytes, 0,
								 NULL, NULL, NULL);
	#else
		el_data_in_pde = (SCALAR *)malloc(el_data_in_bytes);
		el_data_in_jac = (SCALAR *)malloc(el_data_in_jac_bytes);
	#endif

		retval |= clSetKernelArg(kernel_jac, 2, sizeof(cl_mem), (void *) &cmDevBufInJac);

		if (retval != CL_SUCCESS) {
			      printf("Failed to Set the kernel_jac argument %d.\n", 2);
			      exit(0);
			    }

		retval |= clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *) &cmDevBufIn);
	...
	const size_t jac_data_bytes = work_group_size_jac * nr_elems_per_kernel_jac * 10 * sizeof(SCALAR);
	cl_mem jac_dat_dev;
	jac_dat_dev = clCreateBuffer(context, CL_MEM_READ_WRITE, jac_data_bytes, NULL, NULL);

	retval |= clSetKernelArg(kernel_jac, 3, sizeof(cl_mem), (void *) &jac_dat_dev);
	...
	retval |= clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *) &jac_dat_dev);
    ...
    retval |= clSetKernelArgSVMPointer(kernel, 5, el_data_out);
...
//kew*/
	...
	nr_elems_per_wg_jac =
	  ceil(((double)nr_elems_this_kercall) / nr_work_groups_jac / work_group_size_jac) * work_group_size_jac;
...
/*kbw*/
      ...
      printf("\tnr_work_groups %d, work_group_size %d, nr_threads %d\n",
	     nr_work_groups, work_group_size, nr_threads );
      printf("\tnr_elems: nr_elems_per_work_group_jac %d (%d), nr_elems_per_thread_jac %.1f (%.1f)(%.1f)\n",
      	     nr_elems_per_wg_jac, nr_elems_per_wg_jac*nr_work_groups_jac,
      	     nr_elems_per_thread_jac, nr_elems_per_thread_jac*nr_threads_jac,
      	     nr_elems_per_thread_jac*work_group_size_jac*nr_work_groups_jac);
            printf("\tnr_work_groups_jac %d, work_group_size_jac %d, nr_threads_jac %d\n",
      	     nr_work_groups_jac, work_group_size_jac, nr_threads_jac );
//kew*/
#ifdef TUNING
	...
	if(nr_elems_per_wg_jac*nr_work_groups_jac != nr_elems_per_thread_jac*nr_threads_jac){
	  printf("nr_elems_per_wg_jac*nr_work_groups_jac %d != %d nr_elems_per_thread_jac*nr_threads_jac\n",
		 nr_elems_per_wg_jac*nr_work_groups_jac, nr_elems_per_thread_jac*nr_threads_jac );
	  printf("Exiting!\n");
	  exit(-1);
	}
	if(nr_elems_per_wg_jac*nr_work_groups_jac < nr_elems_this_kercall){
	  printf("nr_elems_per_wg_jac*nr_work_groups_jac %d < %d nr_elems_this_kercall\n",
		 nr_elems_per_wg_jac*nr_work_groups_jac, nr_elems_this_kercall );
	  printf("Exiting!\n");
	  exit(-1);
	}
...
// FILL MEMORY OBJECTS WITH DATA AND SEND INPUT DATA TO GPU MEMORY
      ...
      execution_parameters_host[0] = nr_elems_per_thread_jac;
      execution_parameters_host[2] = nr_elems_per_work_group;
...
//	    shape_fun_host[ki*4*num_shap+4*i+3] = base_dphiz_ref[i];
		  shape_fun_host[ki*4*num_shap+i] = base_phi_ref[i];
		  shape_fun_host[ki*4*num_shap+1*num_shap+i] = base_dphix_ref[i];
		  shape_fun_host[ki*4*num_shap+2*num_shap+i] = base_dphiy_ref[i];
		  shape_fun_host[ki*4*num_shap+3*num_shap+i] = base_dphiz_ref[i];
...
//#ifndef OPENCL_HSA
      memset(el_data_in_pde, 0, el_data_in_bytes);
      memset(el_data_in_jac, 0, el_data_in_jac_bytes);
//#endif
      //printf("tutajX\n");
...
//printf("tutaj2\n");
	for(i=0;i<num_geo_dofs;i++){
		  el_data_in_jac[position_geo_dofs] = geo_dofs[3*i];
		  el_data_in_jac[position_geo_dofs+1] = geo_dofs[3*i+1];
		  el_data_in_jac[position_geo_dofs+2] = geo_dofs[3*i+2];
		  position_geo_dofs += 3;
	}
...
/*kew*/
	...
	el_data_in_pde[position_coeff+0] = axx[0];
	el_data_in_pde[position_coeff+1] = axy[0];
	el_data_in_pde[position_coeff+2] = axz[0];
	el_data_in_pde[position_coeff+3] = ayx[0];
	el_data_in_pde[position_coeff+4] = ayy[0];
	el_data_in_pde[position_coeff+5] = ayz[0];
	el_data_in_pde[position_coeff+6] = azx[0];
	el_data_in_pde[position_coeff+7] = azy[0];
	el_data_in_pde[position_coeff+8] = azz[0];
	
	el_data_in_pde[position_coeff+9] = bx[0];
	el_data_in_pde[position_coeff+10] = by[0];
	el_data_in_pde[position_coeff+11] = bz[0];
	
	el_data_in_pde[position_coeff+12] = tx[0];
	el_data_in_pde[position_coeff+13] = ty[0];
	el_data_in_pde[position_coeff+14] = tz[0];
	
	el_data_in_pde[position_coeff+15] = cval[0];
	
	el_data_in_pde[position_coeff+16] = qx[0];
	el_data_in_pde[position_coeff+17] = qy[0];
	el_data_in_pde[position_coeff+18] = qz[0];
	
	el_data_in_pde[position_coeff+19] = sval[0];
	position_coeff += 20;
...
//kew*/
      ...
      //ssert(packed_bytes<=el_data_in_bytes);
      // writing input data to global GPU memory


      // run first kernel
      clEnqueueWriteBuffer(command_queue, cmDevBufInJac, CL_FALSE, 0,
      			   el_data_in_jac_bytes, el_data_in_jac, 0, NULL, NULL);

#ifdef TIME_TEST
      clFinish(command_queue);
      t_end = time_clock();
      printf("\nEXECUTION TIME: sending el_data_in_jac to GPU memory %lf (speed %lf [GB/s])\n",
	     t_end-t_begin, el_data_in_jac_bytes*1.0e-9/(t_end-t_begin));
#ifdef TUNING
      if(line_count==0)
	{
	fprintf(headf,"sending el_data_in_jac to GPU memory,[GB/s],");
	}
      fprintf(resf,"%lf,%lf,",
	     t_end-t_begin, el_data_in_jac_bytes*1.0e-9/(t_end-t_begin));
#endif
      ...
      // FINALLY EXECUTE THE KERNEL
       ...
       size_t globalWorkSizeJac[1] = { 0 };
       size_t localWorkSizeJac[1] = { 0 };
       globalWorkSizeJac[0] = nr_threads_jac ;
       localWorkSizeJac[0] = work_group_size_jac ;

       cl_event ndrEvtJac;

       // Queue the kernel up for execution
       retval = clEnqueueNDRangeKernel(command_queue, kernel_jac, 1, NULL,
       				      globalWorkSizeJac, localWorkSizeJac,
       				      0, NULL,  &ndrEvtJac);


 #ifdef TIME_TEST
       clWaitForEvents(1, &ndrEvtJac);
       clFinish(command_queue);

       t_end = time_clock();

       // Get kernel profiling info
       cl_ulong startTimeJac;
       cl_ulong endTimeJac;
       clGetEventProfilingInfo(ndrEvtJac,
       ...
       clGetEventProfilingInfo(ndrEvtJac,
 			      ...
 			      &endTimeJac,
 			      0);
       double time_internal_jac = ((double)endTimeJac - (double)startTimeJac)*1.0e-9;

       double kernel_execution_time_jac=0.0;
       kernel_execution_time_jac = t_end-t_begin;
       printf("EXECUTION TIME: executing jacobian kernel: %lf (internal %lf)\n",
 	     kernel_execution_time_jac, time_internal_jac);
 #ifdef TUNING
       if(line_count==0)
     	{
     	fprintf(headf,"executing kernel_jac,internal_jac,");
     	}
       fprintf(resf,"%lf,%lf,",
       	     kernel_execution_time_jac, time_internal_jac);
 #endif
...
#ifndef OPENCL_HSA
      clEnqueueWriteBuffer(command_queue, cmDevBufIn, CL_FALSE, 0,
      			   el_data_in_bytes, el_data_in_pde, 0, NULL, NULL);
#endif
...
//      for(i=0;i<WORK_GROUP_SIZE;i++)
//      {
//    	  //int ind=i*2;
//    	  printf("thr-%d = %.0lf oper\n",i,el_data_out[i]);
//    	  sum_up+=el_data_out[i];
//      }
//      printf("Operacji = %lf\n",sum_up);
...
#ifdef INV
			 SCALAR* el_data_out_tmp2 =(SCALAR*) malloc(num_shap*num_shap*sizeof(SCALAR));
			 //memcpy(el_data_out_tmp2,el_data_out,el_data_out_bytes);
			 int wg,thr,elem;
			 int offset, elem_index;

			 for(wg=0;wg<nr_work_groups;wg++)
			 {
#ifndef ONE_EL_ONE_WORKGROUP
				 for(ielem=0;ielem<(nr_elems_per_work_group/work_group_size);ielem++)
#else
...
#endif
				 {
					 ...
					 {
#ifdef ONE_EL_ONE_WORKGROUP
						 elem_index=wg*nr_elems_per_work_group+ielem;
#else
						 elem_index=wg*nr_elems_per_work_group+ielem*work_group_size+thr;
#endif
						 offset=elem_index*(num_shap*num_shap+num_shap);
						 for(i=0;i<num_dofs;i++)
						 {
							//index_GPU2+=i;
							for(j=0;j<num_dofs;j++)
							{
								//printf("pod %d zapisuje %d\n",j+i*num_dofs,index_GPU2+i+j*num_dofs);
								el_data_out_tmp2[j+i*num_dofs]=el_data_out[offset+i+j*num_dofs];
							}
						 }

						for(i=0;i<num_dofs*num_dofs;i++)
						{
							el_data_out[offset+i]=el_data_out_tmp2[i];
						}
					 }
				 }
			 }
...
/* #pragma omp parallel for default(none) firstprivate(nr_work_groups, nr_elems_per_work_group, nr_parts_of_stiff_mat, nr_iter_within_part, num_shap, work_group_size, kernel_index, nreq, num_dofs, stiff_mat, el_data_out) shared(first_elem_kercall) */
	...
	int nr_entries=0;
	  ...
	  for(iiter = 0; iiter < last_iter; iiter++ ){
		...
		//stiff_mat[index_CPU]=idofs*1000+jdofs;
/*kbw*/
		double tol = 1.e-4;
...
//to dodane bo jest wiecej niz trzeba
			if(index_GPU_v>=num_dofs*num_dofs)
				break;
...
/*kbw*/
		...
		if((fabs(rhs_vect[index_CPU_v]) <  tol && 
		  ...
		  printf("index GPU_v %"
				  "d,\tindex CPU_v %d,\tvalue GPU_v %12.6lf,\tvalue CPU_v %12.6lf\n",
...
#pragma omp critical(assembling)
    ...
    clSVMFree(context,el_data_out);
		...
		free(el_data_out);
	#else
		clEnqueueUnmapMemObject(command_queue, cmPinnedBufIn, el_data_in_pde, 0, NULL, NULL);
		clReleaseMemObject(cmPinnedBufIn);
		clEnqueueUnmapMemObject(command_queue, cmPinnedBufInJac, el_data_in_jac, 0, NULL, NULL);
		clReleaseMemObject(cmPinnedBufInJac);
		...
		clReleaseMemObject(cmDevBufInJac);
