...
#ifdef OUTSIDE 
	#define SIZE NENTPT*WORK_GROUP_SIZE
#endif
...
#elif defined HEAT
  #define NR_COEFFS_SENT_PER_ELEMENT (num_shap+1)
...
// DEFINITIONS DEPENDENT ON OPTIMIZATION OPTIONS
  ...
  SCALAR pde_coeff[num_shap];
	...
	#define WORKSPACE_SIZE_FOR_JACOBIAN_DATA num_gauss*10
  	__shared__ SCALAR workspace_jac[WORKSPACE_SIZE_FOR_JACOBIAN_DATA]; //
#endif
  ...
  #define WORKSPACE_SIZE_FOR_STIFF_MAT (num_shap*(num_shap+1))
#else
...
#define TMP_SIZE (WORKSPACE_SIZE_FOR_PDE_COEFF+WORKSPACE_SIZE_FOR_SHAPE_FUN+WORKSPACE_SIZE_FOR_STIFF_MAT)
...
//__shared__ SCALAR stiff_mat_ws[NENTPT*WORK_GROUP_SIZE];
...
if(jdof<num_shap)
{
    	...
    	SCALAR stiff_mat[NENTPT/4];
    	SCALAR stiff_mat1[NENTPT/4];
    	SCALAR stiff_mat2[NENTPT/4];
    	SCALAR stiff_mat3[NENTPT/4];

		for(i=0;i<NENTPT/4;i++)
...
//************************* LOOP OVER INTEGRATION POINTS ************************//
    ...
    for(igauss = 0; igauss < num_gauss; igauss++){
    	...
    	SCALAR jac[10];
    			...
    			//__syncthreads(); // !!!!!!!!!!!!!!!!!!!!!!
//    			if(igauss==0 && group_id==0 && thread_id==1 && element_index==79)
//				{
//					printf("wg=%d,elem_ind=%d,thread=%d,offset=%d\n",group_id,element_index,thread_id,offset);
//					for(i=0;i<10;i++)
//					{
//						printf("jac[%d]=%lf\n",i,jac[i]);
...
//    			if(igauss==0 && group_id==0 && thread_id==0 && element_index==81)
//				{
//					printf("wg=%d,elem_ind=%d,thread=%d,offset=%d\n",group_id,element_index,thread_id,offset);
//					for(i=0;i<10;i++)
//					{
//						printf("jac[%d]=%lf\n",i,jac[i]);
//						printf("workspace_jac[%d]=%lf\n",igauss*10+i,workspace_jac[igauss*10+i]);
//					}
//				}
			#else
    			//dorobic
...
if(thread_id<num_shap)
{
	...
	temp1 = shpfun_ref[igauss*4*num_shap+WORK_GROUP_SIZE+thread_id];
	temp2 = shpfun_ref[igauss*4*num_shap+2*WORK_GROUP_SIZE+thread_id];
	temp3 = shpfun_ref[igauss*4*num_shap+3*WORK_GROUP_SIZE+thread_id];
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF IDOF SHAPE FUNCTION ******//
	  ...
	  SCALAR shp_fun_u = shpfun_ref[igauss*4*num_shap+jdof];
	  ...
	  // read proper values of shape functions and their derivatives
          SCALAR shp_fun_u = shpfun_ref[igauss*4*num_shap+thread_id];
...
//-------------------------------------------------------------
	  ...
	  for(i=0;i<NENTPT/4;i++)  //NENTPT loop
	  {
	  	idof=(aux_offset-jdof*num_shap)+i;
...
//*** ACTUAL CALCULATIONS FOR LOAD VECTOR (AND IDOF SHAPE FUNCTION) ***//
	  ...
	  workspace[thread_id*(num_shap*(num_shap+1)+PADDING)+num_shap*num_shap+idof] += (
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF JDOF SHAPE FUNCTION ******//
	  ...
	  SCALAR shp_fun_v = shpfun_ref[igauss*4*num_shap+idof];
...
//********* ACTUAL FINAL CALCULATIONS FOR SM ENTRY  *********//
	...
	workspace[thread_id*(num_shap*(num_shap+1)+PADDING)+idof*num_shap+jdof] += (
...
//	{
//	  printf("thread_id=%d, idof2=%d, idof=%d, jdof=%d, temp4=%lf,fun_v_derx=%lf, vol=%lf \n", thread_id, idof2, idof,jdof, temp4,fun_v_derx,vol);
...
//stiff_mat1
	  for(i=0;i<NENTPT/4;i++)  //NENTPT loop
	  {
	  	idof=(aux_offset-jdof*num_shap)+NENTPT/4+i;
...
//stiff_mat2
	  for(i=0;i<NENTPT/2;i++)  //NENTPT loop
	  {
	  	idof=(aux_offset-jdof*num_shap)+NENTPT/2+i;
...
//-------------------------------------------------------------
	  ...
	  for(i=0;i<NENTPT/4;i++)  //NENTPT loop
	  {
	  	idof=(aux_offset-jdof*num_shap)+3*NENTPT/4+i;
...
//          {
    ...
    for(i=0;i<NENTPT/4;i++)
    {
    	...
    	//stiff_mat_out[aux_offset+i*NENTPT+thread_id] = stiff_mat[i*NENTPT+thread_id];//jdof*1000+idof;//stiff_mat[i];
    	//stiff_mat_ws[i*NENTPT+thread_id] = stiff_mat[i];
    	stiff_mat_out[aux_offset+i*NENTPT+thread_id] = stiff_mat[i];
    	stiff_mat_out[aux_offset+NENTPT/4+i*NENTPT+thread_id] = stiff_mat1[i];
    	stiff_mat_out[aux_offset+NENTPT/2+i*NENTPT+thread_id] = stiff_mat2[i];
    	stiff_mat_out[aux_offset+3*NENTPT/4+i*NENTPT+thread_id] = stiff_mat3[i];
...
//    stiff_mat_out[aux_offset+thread_id] = stiff_mat[0];
//    stiff_mat_out[aux_offset+1*NENTPT+thread_id] = stiff_mat[1];
//    stiff_mat_out[aux_offset+2*NENTPT+thread_id] = stiff_mat[2];
//    stiff_mat_out[aux_offset+3*NENTPT+thread_id] = stiff_mat[3];
//    stiff_mat_out[aux_offset+4*NENTPT+thread_id] = stiff_mat[4];
//    stiff_mat_out[aux_offset+5*NENTPT+thread_id] = stiff_mat[5];
//    stiff_mat_out[aux_offset+6*NENTPT+thread_id] = stiff_mat[6];
//    stiff_mat_out[aux_offset+7*NENTPT+thread_id] = stiff_mat[7];
//    stiff_mat_out[aux_offset+8*NENTPT+thread_id] = stiff_mat[8];
//    stiff_mat_out[aux_offset+9*NENTPT+thread_id] = stiff_mat[9];
//    stiff_mat_out[aux_offset+10*NENTPT+thread_id] = stiff_mat[10];
//    stiff_mat_out[aux_offset+11*NENTPT+thread_id] = stiff_mat[11];
//    stiff_mat_out[aux_offset+12*NENTPT+thread_id] = stiff_mat[12];
//    stiff_mat_out[aux_offset+13*NENTPT+thread_id] = stiff_mat[13];
//    stiff_mat_out[aux_offset+14*NENTPT+thread_id] = stiff_mat[14];
//    stiff_mat_out[aux_offset+15*NENTPT+thread_id] = stiff_mat[15];
...
//    for(i=0;i<NENTPT;i++)
//	{
//		stiff_mat_out[aux_offset+i*NENTPT+thread_id] = stiff_mat_ws[i*NENTPT+thread_id];//jdof*1000+idof;//stiff_mat[i];
//	}
