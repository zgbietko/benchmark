/************************************************************************
File pds_laplace_std_adapt - contains routines for adaptations, including
                             Zienkiewicz-Zhu error estimation
                             and nodal first derivative patch recovery
...
)
{
  ...
  if(Mode==PDC_ADAPT_EXACT){
    ...
    printf("Unknown error indicator in pdd_laplace_std_adapt!\n");
...
/* #pragma omp threadprivate (wg) */
  ...
  int base;		/* type of basis functions  */
  int base_old=-1;		/* type of basis functions  */
...
/*++++++++++++++++ executable statements ++++++++++++++++*/
  ...
  clock_time = time_clock()-clock_time;
  printf("Derivative recovery - time %lf\n", clock_time);
