/************************************************************************
File pds_ns_supg_ale_ls_intf.c - interface between the problem dependent
...
Two local procedures :
  ...
  pdr_ns_supg_ale_comp_el_stiff_mat - to construct a stiffness matrix and a load
                            vector for an element (used by pdr_comp_stiff_mat) 

  pdr_ns_supg_ale_comp_fa_stiff_mat - to construct a stiffness matrix and a load
...
#include "../include/pdh_ns_supg_ale.h"	/* USES */
...
/*kew*/
  ...
  if(ref_temperature<=0){
    ...
    qparams.material_idx = Mat_num;	// query by material index ...
    ...
    qparams.query_type = QUERY_NS_SUPG_POINT;
    //2.get query results
    ...
    if(problem->ctrl.dynamic_viscosity>0){ 
      dyn_visc = problem->ctrl.dynamic_viscosity;
    }
  }
#ifdef TURBULENTFLOW
