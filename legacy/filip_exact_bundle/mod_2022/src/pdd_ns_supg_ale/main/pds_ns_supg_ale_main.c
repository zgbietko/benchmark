/************************************************************************
File pds_ns_supg_ale_main.c - driver for ns_supg module - approximation
...
Contains definition of routines:
  main
  pdr_ns_supg_ale_init - initialize problem data
...
#include "../include/pdh_ns_supg_ale.h"	
...
//#include "../main/pds_ns_supg_ale_util.c"
...
int pdv_ns_supg_ale_current_problem_id=PDC_NS_SUPG_ID;/* ID of the current problem */
// problem structure for ns_supg module
pdt_ns_supg_problem pdv_ns_supg_problem;

  /* time measurements */
  double timer_all = 0.0;
  double pdv_ns_supg_ale_timer_all = 0.0;
  double pdv_ns_supg_ale_timer_pdr_comp_el_stiff_mat = 0.0;
  double pdv_ns_supg_ale_timer_pdr_comp_fa_stiff_mat = 0.0;
...
/*---------------------------------------------------------
  pdr_ns_supg_ale_init - to initialize problem data 
                          (including mesh and two fields)
----------------------------------------------------------*/
extern int pdr_ns_supg_ale_init(
  char* Work_dir,
...
/*++++++++++++++++ executable statements ++++++++++++++++*/
  ...
  pdv_ns_supg_ale_current_problem_id = problem_id;
...
/*++++++++++++++++ executable statements ++++++++++++++++*/
  ...
  pdr_ns_supg_ale_init(work_dir, interactive_input, interactive_output);
  ...
  do {
    ...
    if (c == 's') {
      ...
      fprintf(interactive_output, 
	      "\nBeginning solution of ns_supg problem\n\n");

      timer_all = time_clock();

      pdr_ns_supg_ale_time_integration(work_dir, 
			   interactive_input, interactive_output);
    ...
    /*------------------------------------------------------------*/
      ...
      pdr_ns_supg_ale_ZZ_error(work_dir, 
			   interactive_input, interactive_output);
    ...
    /*------------------------------------------------------------*/
      ...
      utr_manual_refinement(pdv_ns_supg_ale_current_problem_id, 
    ...
    /*------------------------------------------------------------*/
      ...
      pdr_ns_supg_ale_dump_data(work_dir, 
			   interactive_input, interactive_output);
    ...
    /*------------------------------------------------------------*/
      ...
      pdv_ns_supg_ale_current_problem_id = PDC_NS_SUPG_ID;
      utr_test_refinements(pdv_ns_supg_ale_current_problem_id, work_dir, 
			   interactive_input, interactive_output );
  ...
  pdv_ns_supg_ale_current_problem_id = PDC_NS_SUPG_ID;
  iaux=3; field_id =  pdr_ctrl_i_params(pdv_ns_supg_ale_current_problem_id,iaux);
...
/*------------------------------------------------------------
pdr_ns_supg_ale_init - read problem data
------------------------------------------------------------*/
int pdr_ns_supg_ale_init(
  char *Work_dir,
  FILE *Interactive_input,
  FILE *Interactive_output
)
{
  ...
  fprintf(Interactive_output, "\tmaterials_file:\t\t\t%s\n", 
  ...
  fprintf(Interactive_output,"\tpenalty for Dirichlet BCs:\t%lf\n\n",
  ...
  // 's' - for standard continuous basis functions
  // 'i' - for initializing using function pdr_ns_supg_ale_initial_condition
    pdv_ns_supg_problem.ctrl.field_id = utr_initialize_field(
