...
//FK PAdding
    int one_el_stiff_mat_size = WORK_GROUP_SIZE*WORK_GROUP_SIZE;
    int one_el_load_vec_size = WORK_GROUP_SIZE;
...
// FILL MEMORY OBJECTS WITH DATA AND SEND INPUT DATA TO GPU MEMORY
      ...
      if( kernel_version_alg==ONE_EL_ONE_WORKGROUP_KERNEL)
      {
    	  execution_parameters_host[0] = nr_elems_per_work_group;
      }
...
//	    shape_fun_host[ki*4*num_shap+4*i+3] = base_dphiz_ref[i];
		  ...
		  shape_fun_host[ki*4*num_shap+1*WORK_GROUP_SIZE+i] = base_dphix_ref[i];
		  shape_fun_host[ki*4*num_shap+2*WORK_GROUP_SIZE+i] = base_dphiy_ref[i];
		  shape_fun_host[ki*4*num_shap+3*WORK_GROUP_SIZE+i] = base_dphiz_ref[i];
