...
#ifdef OPENCL_CPU
#define NR_WORK_GROUPS_PER_COMP_UNIT  16 // ????
#elif defined OPENCL_GPU
#define NR_WORK_GROUPS_PER_COMP_UNIT  16 // TESLA according to Nvidia Profiler
#elif defined OPENCL_PHI
...
// For CPU better result is when we don't use pinned buffer
    ...
    //Xe
    SCALAR* el_data_in = (SCALAR *) clSVMAlloc(context,CL_MEM_READ_ONLY,el_data_in_bytes,NULL);


    if(el_data_in==NULL)
        	printf("Error 536! in allocating buffer");
	...
	//Xe
	SCALAR* el_data_out = (SCALAR *) clSVMAlloc(context,CL_MEM_WRITE_ONLY,el_data_out_bytes,NULL);
    if(el_data_out==NULL)
    	printf("Error 53621! in allocating buffer");
...
// AND OUTPUT DATA TRANSFERRED BACK TO HOST COMPUTER MEMORY
    ...
    //printf("!!!!!!!!!!!!!!nr_elems_per_kercall=%d, nr_elems_this_color=%d, nr_elems_per_kernel=%d\n",nr_elems_per_kercall,nr_elems_this_color,nr_elems_per_kernel);
...
//kew*/
      printf("!!!!!!!!!!!!!!nr_elems_per_kercall=%d, nr_elems_this_color=%d\n",nr_elems_per_kercall,nr_elems_this_color);
...
//kew*/
	...
	if( kernel_version_alg==ONE_EL_ONE_WORKGROUP_KERNEL) {
	  ...
	  printf("Too low number of elements per workgroup %d \n", nr_elems_per_work_group );
	  printf("Check parameters of execution. Exiting!!!TU\n");
	  exit(-1);
	}
