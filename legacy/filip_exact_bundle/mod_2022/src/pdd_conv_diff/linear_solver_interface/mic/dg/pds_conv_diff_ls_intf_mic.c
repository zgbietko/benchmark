...
#define STRIDE 1 //for AVX registers with stride version(with other set to one)
...
//		for(i=0;i<ngauss;i++)
//		{
//			printf("xg[%d]=%lf,xg[%d]=%lf,xg[%d]=%lf,wg[%d]=%lf\n",3*i,xg[3*i],3*i+1,xg[3*i+1],3*i+2,xg[3*i+2],i,wg[i]);
//		}
	...
	// This is the version when we send only necessary variables but we do not care about sending
		...
		else
		{
			for(i=0; i<ngauss; i++){
			  ...
			  gauss_dat_host[3*ngauss+i] = wg[i];
			}
		}
	}
...
//			shape_fun_host[ki*4*num_shap+4*i+3] = base_dphiz_ref[i];
			...
			//printf("shape_fun_host[%d]=%lf\n",3*ngauss*PAD+ki*PAD+i,shape_fun_host[3*ngauss*PAD+ki*PAD+i]);

			//printf("i=%d,base_phi=%lf, base_dphix=%lf, base_dphiy=%lf, base_dphiz=%lf\n",i, base_phi_ref[i], base_dphix_ref[i], base_dphiy_ref[i], base_dphiz_ref[i]);
...
#define INV
 ...
 for(ielem=0; ielem < Nr_elems_check; ielem++){
	// element ID
	double *stiff_mat1 = (double *)malloc(num_dofs*num_dofs*sizeof(double));

	  /* initialize the matrices to zero */
	  for(i=0;i<num_dofs*num_dofs;i++) stiff_mat1[i]=0.0;

	  int offset2=ielem*(one_el_stiff_mat_size+one_el_load_vec_size);

	  int index_GPU2=offset2;
	  int index_rev=0;
		...
		for(i=0;i<num_dofs;i++)
		{
			...
			for(j=0;j<num_dofs;j++)
			{
				...
				stiff_mat1[j+i*num_dofs]=el_data_out[index_GPU2+i+j*num_dofs];
			}
		}
		...
		{
			el_data_out[index_GPU2+i]=stiff_mat1[i];
		}
 }
 ...
 for(ielem=0; ielem < Nr_elems_check; ielem++){
 	 ...
 	 int offset3=ielem*(one_el_stiff_mat_size+one_el_load_vec_size);
     /*kbw*/
         printf("\nElement %d: Stiffness matrix:\n",ielem);
           for (i=0;i<num_dofs;i++) {
     	for (j=0;j<num_dofs;j++) {
     	  printf("%20.12lf",el_data_out[offset3+i+j*num_dofs]);
     	}
     	printf("\n");
           }

           offset3=ielem*(one_el_stiff_mat_size+one_el_load_vec_size)+one_el_stiff_mat_size;

           printf("Element %d: Rhs_vect:\n",ielem);
           for (i=0;i<num_dofs;i++) {
     	printf("%20.12lf",el_data_out[offset3+i]);
           }
           printf("\n");
           getchar();
     /*kew*/
...
//	  			}
				...
				//if(Comp_sm!=PDC_NO_COMP) Comp_sm += 3;
	  			...
	  			{
	  				//printf("Przed assemblingiem w problemie: Problem_id=%d, level_id=%d, SIC_SOLVE=%d, nr_dof_ent=%d,rewrite=%c\n",Problem_id, level_id, Comp_type,
	  				//				   nr_dof_ent, rewrite);
