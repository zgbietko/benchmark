...
// how many workgroups will be enough to make GPU cores (comp_units) busy?
    ...
    //8 to za dużo tutaj
        int nr_work_groups_jac = 4 * max_num_comp_units;
...
// the page-locked memory for faster transfers
    ...
    SCALAR* el_data_in_jac = NULL; // host input buffer (standard array)
...
#define PROFILING
    ...
    SCALAR *cmDevBufInCUDA_JAC;

    //cudaHostAlloc((void**)&el_data_in, el_data_in_bytes, cudaHostAllocDefault);
    cudaMalloc((void**)&cmDevBufInCUDA, el_data_in_bytes);
    cudaMalloc((void**)&cmDevBufInCUDA_JAC, el_data_in_jac_bytes);

    err = cudaMemcpy(cmDevBufInCUDA, el_data_in_pde,
		     el_data_in_bytes, cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
      {
	printf("Memcpy fail1, error : %d, %s \n", err,  cudaGetErrorString(err));
	exit(0);
      }

    err = cudaMemcpy(cmDevBufInCUDA_JAC, el_data_in_jac,
    		     el_data_in_jac_bytes, cudaMemcpyHostToDevice);
	if (err != cudaSuccess)
	  {
	printf("Memcpy fail2, error : %d, %s \n", err,  cudaGetErrorString(err));
	exit(0);
	  }
	...
	SCALAR *cmDevBuf_JAC;
	cudaMalloc((void**)&cmDevBuf_JAC, jac_data_bytes);

//	const size_t shp_fun_dev_bytes = work_group_size_jac * nr_elems_per_kernel_jac * (num_shap * ngauss * 4 + ngauss) * sizeof(SCALAR);
//	SCALAR *cmDevBuf_SHP;
//	cudaMalloc((void**)&cmDevBuf_SHP, shp_fun_dev_bytes);
	...
	fprintf(headf,"sending el_data_in to GPU memory,[GB/s],");
      ...
      cudaEvent_t startJACKernelEvent,endJACKernelEvent;
      cudaEvent_t startJACMemcpyEvent,endJACMemcpyEvent;

      cudaEventCreate(&startJACKernelEvent);
      cudaEventCreate(&endJACKernelEvent);
      cudaEventCreate(&startJACMemcpyEvent);
      cudaEventCreate(&endJACMemcpyEvent);

      cudaEventRecord(startJACKernelEvent, 0);
      ...
      int gridSizeJac = nr_work_groups_jac;
      int blockSizeJac = work_group_size_jac;

      cudaSetDevice(device_index);

      cudaDeviceSynchronize();

//      invokeJacKernel( gridSizeJac,
//                    blockSizeJac,
...
//                    cmDevBuf_SHP);
      ...
      cudaEventRecord(endJACKernelEvent, 0);
      ...
      cudaEventSynchronize(endJACKernelEvent);


      float JackernelExecutionTimeInMs = 0.0f;
      cudaEventElapsedTime(&JackernelExecutionTimeInMs, startJACKernelEvent, endJACKernelEvent);
      cudaEventDestroy(startJACKernelEvent);
      cudaEventDestroy(endJACKernelEvent);
...
//      double time_internal = ((double)endTime - (double)startTime)*1.0e-9;
      ...
      double jac_kernel_execution_time=0.0;
      jac_kernel_execution_time = t_end-t_begin;

      printf("EXECUTION TIME: executing jacobian kernel: %lf \n", JackernelExecutionTimeInMs/1000.);
      ...
      fprintf(resf,"%lf,%lf,",jac_kernel_execution_time,
...
//#define TESTING_GPU_TRANSFER
    ...
    total_time += t_end-t_begin;
      ...
      //cudaFree(cmDevBuf_SHP);
      cudaFree(cmDevBuf_JAC);
