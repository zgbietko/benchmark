...
//	  	}
	  ...
	  // read proper values of shape functions and their derivatives
          SCALAR shp_fun_u = shpfun_ref[igauss*4*num_shap+thread_id];
          SCALAR fun_u_derx = tab_fun_u_derx[thread_id];
          SCALAR fun_u_dery = tab_fun_u_dery[thread_id];
          SCALAR fun_u_derz = tab_fun_u_derz[thread_id];
...
//*** ACTUAL CALCULATIONS FOR LOAD VECTOR (AND IDOF SHAPE FUNCTION) ***//
	  ...
	  workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+num_dofs*num_dofs+thread_id] += (
...
//********* ACTUAL FINAL CALCULATIONS FOR SM ENTRY  *********//
	...
	workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+jdof] += (
...
//	{
//	  printf("thread_id=%d, thread_id2=%d, idof=%d, jdof=%d, temp4=%lf,fun_v_derx=%lf, vol=%lf \n", thread_id, idof2, idof,jdof, temp4,fun_v_derx,vol);
...
//-------------------------------------------------------------
    ...
    //******** WRITING OUTPUT SM AND LV TO GLOBAL MEMORY ********//
        ...
        offset = element_index*(num_shap_pad*num_shap_pad+num_shap_pad);
//if(thread_id<num_shap)
{
        for(jdof=0; jdof < num_shap_pad; jdof++)
		{
    	    ...
    	    stiff_mat_out[offset+jdof*num_shap_pad+thread_id] = stiff_mat[jdof];
    	...
    	workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+num_dofs*num_dofs+i];
          ...
          if(thread_id<num_shap)
               stiff_mat_out[offset+num_shap*num_shap+thread_id] = load_vec[0];
      ...
      #else
    	    if(idof<num_shap)
    	    	stiff_mat_out[offset+jdof] = stiff_mat[jdof];
    	    	//__syncthreads();
