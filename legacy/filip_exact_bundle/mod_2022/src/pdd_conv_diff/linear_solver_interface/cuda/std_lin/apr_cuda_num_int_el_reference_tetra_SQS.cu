...
// DEFINITIONS DEPENDENT ON OPTIMIZATION OPTIONS
  ...
  #define WORKSPACE_SIZE_FOR_STIFF_MAT (num_dofs+1)
#else
...
int igauss;
int idof, jdof; //to prevent redeclaration of idof
...
//******************** INITIALIZING SM AND LV ******************//
    ...
    for(i = 0; i < (num_dofs+1); i++) {
      workspace[thread_id*((num_dofs+1)+PADDING)+i] = zero;
    }
...
//*** ACTUAL CALCULATIONS FOR LOAD VECTOR (AND IDOF SHAPE FUNCTION) ***//
	  ...
	  workspace[thread_id*(num_dofs+1+PADDING)+num_dofs] += (
...
//********* ACTUAL FINAL CALCULATIONS FOR SM ENTRY  *********//
	...
	workspace[thread_id*(num_dofs+1+PADDING)+jdof] += (
