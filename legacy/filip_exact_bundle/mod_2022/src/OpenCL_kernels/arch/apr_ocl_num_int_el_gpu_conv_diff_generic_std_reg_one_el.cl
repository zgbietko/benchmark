...
//	    	if(group_id==1&&thread_id==0&&ielem==0)
//	    		stiff_mat_out[i]=geo_dat_workspace[thread_id*EL_GEO_DAT_SIZE+i];
	    	//ok sprawdzone
...
#else
	    offset=exec_params_workspace[8]*EL_GEO_DAT_SIZE+group_id*nr_elems_per_thread*work_group_size*(NR_PDE_COEFF_MAT+NR_PDE_COEFF_VEC) + thread_id * nr_elems_per_thread * (NR_PDE_COEFF_MAT+NR_PDE_COEFF_VEC) + ielem*(NR_PDE_COEFF_MAT+NR_PDE_COEFF_VEC);
#endif
//	    if(group_id==0&&thread_id==0)
