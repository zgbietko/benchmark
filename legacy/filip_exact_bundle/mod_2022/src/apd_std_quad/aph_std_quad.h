...
#define APC_MAXELVNO_ON_EDGES 9 /* maximal number of vertices on eges of an element with quadratic approximation */
...
typedef struct {
  ...
  int inactive_used_with_constr; /* flag for inactive node used for calculate value of constraint node */ 
...
typedef struct {
  ...
  int nr_dof_ents;  /* number of vertex + active edges */
  ...
  apt_dof_ent *dof_vert; /* pointer to structure with the vectors of vertex */
  apt_dof_ent *dof_edge; /* pointer to structure with the vectors of edge */
