...
def run_pointer_latency(
    ...
    print("=== GPU pointer-chasing latency benchmark (Metal) ===")
        ...
        for kb in sizes_kb:
            ...
            for run_idx in range(runs_per_size):
                ...
                if logger is not None:
                    logger.start()
                elapsed_s = backend.run_pointer_chase(num_elements=n, iters_inner=iters_inner)
                if logger is not None:
                    try:
                        energy_j, power_w = logger.stop()
                    except Exception:
                        energy_j, power_w = 0.0, 0.0
                q = logger.last_quality_metrics if logger is not None else {}
                lat_ns = (elapsed_s * 1e9) / max(1, iters_inner)
                lats.append(lat_ns)
                print(
                ...
                writer.writerow(row)
            mu = stats.mean(lats)
            sd = stats.pstdev(lats) if len(lats) > 1 else 0.0
...
def main() -> None:
    ap = argparse.ArgumentParser(description="GPU pointer-chasing latency benchmark (Metal).")
