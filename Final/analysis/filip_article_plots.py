...
def _variant_digest_values(
    ...
    *,
    ...
    if not combos or not series:
        return []
    best_by_combo: dict[str, float] = {}
    for _operator, ys in series:
        for combo, value in zip(combos, ys):
            fval = float(value)
            if not math.isfinite(fval):
                continue
            current = best_by_combo.get(combo)
            if current is None or fval < current:
                best_by_combo[combo] = fval
    return [(combo, best_by_combo[combo]) for combo in combos if combo in best_by_combo]


def _plot_variant_digest_figure(
    rows: list[dict[str, Any]],
    *,
    operators: list[str],
    out_path: Path,
    ...
    fig, axes = plt.subplots(1, len(VARIANT_ORDER), figsize=(16.2, 5.0), sharey=True)
    has_any_data = False
    all_values: list[float] = []
    for idx, variant in enumerate(VARIANT_ORDER):
        ax = axes[idx]
        points = _variant_digest_values(rows, operators=operators, variant=variant, include_infeasible=include_infeasible)
        if not points:
            ax.text(0.5, 0.5, "Brak danych", transform=ax.transAxes, ha="center", va="center")
            ax.set_axis_off()
            continue
        has_any_data = True
        ranked = sorted(points, key=lambda item: item[1])
        ys = [float(v) for _, v in ranked]
        xs = list(range(len(ys)))
        all_values.extend(ys)

        y_arr = np.array(ys, dtype=float)
        p10, p50, p90 = np.percentile(y_arr, [10.0, 50.0, 90.0])
        q1, q3 = np.percentile(y_arr, [25.0, 75.0])
        iqr = max(0.0, float(q3 - q1))
        out_thr = float(q3 + 1.5 * iqr)
        out_idx = [i for i, val in enumerate(ys) if val > out_thr]
        top_n = min(max(1, top_k), len(ys))

        ax.plot(xs, ys, color="#1d4ed8", linewidth=1.6, marker="o", markersize=2.4)
        ax.scatter(xs[:top_n], ys[:top_n], color="#16a34a", s=24, zorder=4, label=f"top-{top_n}")
        if out_idx:
            ax.scatter(
                out_idx,
                [ys[i] for i in out_idx],
                marker="x",
                color="#b91c1c",
                s=30,
                linewidths=1.1,
        ...
        ax.axhline(float(p50), color="#334155", linestyle="--", linewidth=1.0, label="P50")
        ax.axhline(float(p90), color="#7c3aed", linestyle=":", linewidth=1.0, label="P90")
        _article_axis_style(ax)
        ax.set_title(f"Wariant {_variant_title(variant)}", fontsize=11.5, pad=6)
        ax.set_xlabel("Ranking kombinacji (najlepsze -> gorsze)", fontsize=9.0)
        if idx == 0:
            ax.set_ylabel("Czas [ns]", fontsize=9.0)
        best_combo = ranked[0][0]
        top3 = ", ".join(combo for combo, _ in ranked[:3])
        ax.text(
            0.02,
            0.98,
            (
                ...
                f"outliery={len(out_idx)}\n"
                f"najlepsza: {best_combo}\n"
                f"top3: {top3}"
            ),
            transform=ax.transAxes,
            va="top",
            ...
            fontsize=7.4,
            color="#0f172a",
            bbox={"boxstyle": "round,pad=0.22", "facecolor": "#f8fafc", "edgecolor": "#cbd5e1", "alpha": 0.95},
        )
    ...
    if not has_any_data:
        plt.close(fig)
        return False
    y_bounds = _article_ylim(all_values)
    if y_bounds is not None:
        for ax in axes:
            if ax.has_data():
                ax.set_ylim(*y_bounds)
    fig.suptitle("Filip: skrót wariantów (top-k, percentyle, outliery)", fontsize=13)
    fig.text(
        0.01,
        0.01,
        "Dla każdego wariantu agregacja używa najlepszego czasu dla danej maski bitowej (minimum po wybranych operatorach).",
        fontsize=8.3,
        color="#475569",
    )
...
def generate_article_plots(optimization_dir: Path, *, max_operators: int = 2) -> dict[str, Any]:
    ...
    variant_digest_path = plots_dir / "filip_variant_digest.png"
    if _plot_variant_digest_figure(
        variant_rows,
        operators=operators,
        out_path=variant_digest_path,
        include_infeasible=(method == "filip_original"),
        top_k=8,
    ):
        generated_core.append(str(variant_digest_path))
