...
from analysis.cache_latency import add_cache_boundary_lines
...
SCRIPT_CHOICES: List[str] = [
    ...
    "scripts/build_v3_documentation_pdf.py",
]
...
WORKFLOWS: Dict[str, Dict[str, str]] = {
    ...
    "5. AI acceleration": {
        "id": "ai_accel",
        "description": "AI acceleration probes: matmul across backends plus platform-native AI path checks (Tensor/Core ML/NE where available).",
    },
    "6. Filip original": {
        "id": "filip_original",
    ...
    "8. FEM option validation": {
        "id": "fem_option_validation",
        "description": "Controlled FEM validation probes that mirror the verification-relevant option controls while preserving cross-platform backend support.",
    },
    "9. Profiler correlation": {
        "id": "profiler_correlation",
        "description": "Correlate exact/native FEM results, validation-probe results, and optional exported profiler reports into one summary artifact.",
    },
    "10. Filip firefly": {
        "id": "filip_firefly",
        "description": "Filip thesis-style FEM autotuning with the firefly optimizer and live evaluation progress.",
    },
    "11. Full thesis pipeline": {
        "id": "full_thesis_pipeline",
        "description": "One full Final campaign. Runs CPU/GPU microbenchmarks, real kernels, FEM option validation, Filip portable sweep, autotuning, firefly, exact/replay when available, and profiler correlation as one orchestrated artifact.",
    },
}
...
TEMPLATES: Dict[str, Dict[str, str]] = {
    ...
    "[FEM] Option validation": {
        ...
        "args": (
            ...
            "--backend auto "
            "--repeats 3 "
            "--fem-option-validation-n-elements 4096 "
            "--fem-option-validation-n-qp 6 "
            "--fem-option-validation-workgroup-size 64"
        ),
    },
    "[Analysis] Profiler correlation": {
        "script": "run_workflow.py",
        "args": (
            "--workflow profiler_correlation "
            "--correlation-optimization-dir /path/to/exact_or_native_run "
            "--correlation-fem-option-validation-dir /path/to/fem_option_validation_run"
        ),
    },
    "[Final] Full thesis pipeline": {
        "script": "run_workflow.py",
        "args": (
            ...
            "--iterations 40 "
            "--filip-case prism_pair "
            "--fem-option-validation-n-elements 16384 "
            "--fem-option-validation-n-qp 6 "
            "--fem-option-validation-workgroup-size 64"
        ),
    },
    "[Docs] Build v3 PDFs": {
        "script": "scripts/build_v3_documentation_pdf.py",
        "args": "",
    },
...
class AutotuneGui(tk.Tk):
    def __init__(self) -> None:
        ...
        self.latest_result_payload: Dict[str, Any] = {}
        self.latest_optimization_data: Dict[str, Any] = {}
        self.latest_matrix_data: Dict[str, Any] = {}
        self.latest_fem_validation_data: Dict[str, Any] = {}
        self.latest_profiler_correlation_data: Dict[str, Any] = {}
    ...
    def _build_workflows_tab(self) -> None:
        ...
        self.workflow_filip_export_canonical_bundles_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            top,
            text="Export canonical replay bundles",
            variable=self.workflow_filip_export_canonical_bundles_var,
            command=self._refresh_workflow_description,
        ).grid(row=12, column=0, columnspan=3, sticky="w", pady=(8, 0))

        ttk.Label(top, text="FEM validation ops").grid(row=12, column=2, sticky="w", pady=(8, 0))
        self.workflow_fem_option_validation_ops_var = tk.StringVar(value="laplace,test")
        ttk.Entry(top, textvariable=self.workflow_fem_option_validation_ops_var).grid(
            row=12, column=3, sticky="we", padx=6, pady=(8, 0)
        )
        ttk.Label(top, text="Variants").grid(row=12, column=4, sticky="e", pady=(8, 0))
        self.workflow_fem_option_validation_variants_var = tk.StringVar(value="qss,sqs,ssq")
        ttk.Entry(top, textvariable=self.workflow_fem_option_validation_variants_var, width=20).grid(
            row=12, column=5, sticky="we", padx=6, pady=(8, 0)
        )

        ttk.Label(top, text="FEM validation sizes").grid(row=13, column=0, sticky="w", pady=(8, 0))
        size_row = ttk.Frame(top)
        size_row.grid(row=13, column=1, columnspan=5, sticky="we", padx=6, pady=(8, 0))
        ttk.Label(size_row, text="n_elements").pack(side="left")
        self.workflow_fem_option_validation_n_elements_var = tk.StringVar(value="4096")
        ttk.Entry(size_row, textvariable=self.workflow_fem_option_validation_n_elements_var, width=10).pack(side="left", padx=(6, 12))
        ttk.Label(size_row, text="n_qp").pack(side="left")
        self.workflow_fem_option_validation_n_qp_var = tk.StringVar(value="6")
        ttk.Entry(size_row, textvariable=self.workflow_fem_option_validation_n_qp_var, width=6).pack(side="left", padx=(6, 12))
        ttk.Label(size_row, text="workgroup").pack(side="left")
        self.workflow_fem_option_validation_workgroup_var = tk.StringVar(value="64")
        ttk.Entry(size_row, textvariable=self.workflow_fem_option_validation_workgroup_var, width=8).pack(side="left", padx=(6, 12))

        ttk.Label(top, text="Correlation dirs").grid(row=14, column=0, sticky="w", pady=(8, 0))
        self.workflow_correlation_optimization_dir_var = tk.StringVar(value="")
        ttk.Entry(top, textvariable=self.workflow_correlation_optimization_dir_var).grid(
            row=14, column=1, columnspan=4, sticky="we", padx=6, pady=(8, 0)
        )
        ttk.Button(top, text="Optimization", command=self._browse_workflow_correlation_optimization_dir).grid(
            row=14, column=5, sticky="w", padx=6, pady=(8, 0)
        )

        ttk.Label(top, text="FEM option validation dir").grid(row=15, column=0, sticky="w", pady=(8, 0))
        self.workflow_correlation_fem_option_validation_dir_var = tk.StringVar(value="")
        ttk.Entry(top, textvariable=self.workflow_correlation_fem_option_validation_dir_var).grid(
            row=15, column=1, columnspan=4, sticky="we", padx=6, pady=(8, 0)
        )
        ttk.Button(top, text="Validation", command=self._browse_workflow_correlation_fem_option_validation_dir).grid(
            row=15, column=5, sticky="w", padx=6, pady=(8, 0)
        )

        ttk.Label(top, text="Profiler reports").grid(row=16, column=0, sticky="w", pady=(8, 0))
        self.workflow_correlation_profiler_reports_var = tk.StringVar(value="")
        ttk.Entry(top, textvariable=self.workflow_correlation_profiler_reports_var).grid(
            row=16, column=1, columnspan=4, sticky="we", padx=6, pady=(8, 0)
        )
        ttk.Button(top, text="Reports", command=self._browse_workflow_correlation_profiler_reports).grid(
            row=16, column=5, sticky="w", padx=6, pady=(8, 0)
        )
        ...
        for var in (
            ...
            self.workflow_fem_option_validation_ops_var,
            self.workflow_fem_option_validation_variants_var,
            self.workflow_fem_option_validation_n_elements_var,
            self.workflow_fem_option_validation_n_qp_var,
            self.workflow_fem_option_validation_workgroup_var,
            self.workflow_correlation_optimization_dir_var,
            self.workflow_correlation_fem_option_validation_dir_var,
            self.workflow_correlation_profiler_reports_var,
        ):
        ...
        self.workflow_filip_export_inputs_var.trace_add("write", lambda *_args: self._refresh_workflow_description())
        self.workflow_filip_export_expected_output_var.trace_add("write", lambda *_args: self._refresh_workflow_description())
        self.workflow_filip_export_canonical_bundles_var.trace_add("write", lambda *_args: self._refresh_workflow_description())
    ...
    def _build_run_tab(self) -> None:
        ...
        ttk.Button(btns, text="Load Latest FEM Validation", command=self._load_latest_fem_option_validation).pack(side="left", padx=2)
        ttk.Button(btns, text="Load Latest Correlation", command=self._load_latest_profiler_correlation).pack(side="left", padx=2)
        ttk.Button(btns, text="Load Latest Full Campaign", command=self._load_latest_thesis_full).pack(side="left", padx=2)
    ...
    def _build_results_tab(self) -> None:
        ...
        ttk.Button(frm_top, text="Load Summary", command=self._load_selected_result_dir).pack(side="left", padx=2)
        ...
        probe_box = ttk.LabelFrame(self.tab_results, text="Probe Support")
        probe_box.pack(fill="x", padx=8, pady=(0, 8))
        self.probe_support_text = ScrolledText(probe_box, height=12, wrap="word")
        self.probe_support_text.pack(fill="x", expand=False, padx=6, pady=6)
        self.probe_support_text.insert(
            "end",
            "Load a profiler-correlation result, or load an exact/native run together with a FEM option validation result, to see probe support here.",
        )
    ...
    def _build_plots_tab(self) -> None:
        ...
        ttk.Button(frm_top, text="Loaded Result Plot", command=self._plot_loaded_result).pack(side="left", padx=2)
        ttk.Button(frm_top, text="Optimization Plot", command=self._show_current_optimization_plot).pack(side="left", padx=2)
        ttk.Button(frm_top, text="Validation Plot", command=self._plot_fem_option_validation).pack(side="left", padx=2)
        ttk.Button(frm_top, text="Correlation Plot", command=self._plot_profiler_correlation).pack(side="left", padx=2)
    ...
    def _format_exact_results_payload(self, payload: Dict[str, Any], summary: Dict[str, Any]) -> str:
        ...
        lines.append("")
        lines.append(f"Experiment class: {summary.get('experiment_class', '')}")
        lines.append(f"Mode label: {summary.get('mode_label', '')}")
        if summary.get("numerical_equivalence"):
            lines.append(f"Numerical equivalence: {summary.get('numerical_equivalence')}")
        ...
        canonical_bundles = summary.get("canonical_replay_bundles") or {}
        if isinstance(canonical_bundles, dict) and canonical_bundles.get("bundle_root"):
            lines.append(f"Canonical replay bundles: {canonical_bundles.get('bundle_root')}")
            if canonical_bundles.get("manifest_path"):
                lines.append(f"Canonical bundle manifest: {canonical_bundles.get('manifest_path')}")
        ...
        if best:
            ...
            if numerical.get("best_decoded_matrix_csv"):
                lines.append(f"Best decoded matrix: {numerical.get('best_decoded_matrix_csv')}")
            if numerical.get("best_decoded_rhs_csv"):
                lines.append(f"Best decoded rhs: {numerical.get('best_decoded_rhs_csv')}")
            if numerical.get("best_decoded_summary"):
                lines.append(f"Best decoded summary: {numerical.get('best_decoded_summary')}")
        ...
        if summary.get("summary_hash"):
            lines.append(f"summary_hash: {summary.get('summary_hash')}")
        return "\n".join(lines)

    def _format_fem_option_validation_payload(self, summary: Dict[str, Any]) -> str:
        lines: List[str] = []
        lines.append("FEM Option Validation")
        lines.append("")
        lines.append(f"Backend: {summary.get('backend', '')}")
        lines.append(f"Execution mode: {summary.get('execution_mode', '')}")
        lines.append(f"Device: {summary.get('device', '')}")
        lines.append(f"Records: {summary.get('records', '')}")
        lines.append(f"n_elements: {summary.get('n_elements', '')}")
        lines.append(f"n_qp: {summary.get('n_qp', '')}")
        lines.append(f"workgroup_size: {summary.get('workgroup_size', '')}")
        lines.append(f"CSV: {summary.get('csv_path', '')}")
        lines.append(f"JSONL: {summary.get('jsonl_path', '')}")
        if summary.get("probe_summary_csv"):
            lines.append(f"Probe summary CSV: {summary.get('probe_summary_csv')}")
        if summary.get("category_summary_csv"):
            lines.append(f"Category summary CSV: {summary.get('category_summary_csv')}")
        if summary.get("markdown_path"):
            lines.append(f"Markdown summary: {summary.get('markdown_path')}")
        plots = summary.get("plots") or {}
        if isinstance(plots, dict) and plots:
            for key, value in sorted(plots.items()):
                lines.append(f"Plot [{key}]: {value}")
        if summary.get("method_note"):
            lines.append(f"Method: {summary.get('method_note')}")
        ...
        if probe_summary:
            ...
            for probe_id, probe in sorted(probe_summary.items()):
                if not isinstance(probe, dict):
                    continue
                lines.append(
                    f"{probe.get('probe_label', probe_id)} [{probe.get('probe_category', '')}]: "
                    f"mean_delta_ratio={probe.get('mean_delta_ratio')} "
                    f"min={probe.get('min_delta_ratio')} max={probe.get('max_delta_ratio')} "
                    f"recommended={probe.get('recommended_state')}"
                )
        ...
        if category_summary:
            ...
            for category, values in sorted(category_summary.items()):
                if not isinstance(values, dict):
                    continue
                lines.append(
                    f"{category}: mean_delta_ratio={values.get('mean_delta_ratio')} "
                    f"min={values.get('min_delta_ratio')} max={values.get('max_delta_ratio')} "
                    f"samples={values.get('samples')}"
                )
        if summary.get("summary_hash"):
            lines.append("")
            lines.append(f"summary_hash: {summary.get('summary_hash')}")
    ...
    def _format_profiler_correlation_payload(self, summary: Dict[str, Any]) -> str:
        lines: List[str] = []
        lines.append("Profiler Correlation")
        lines.append("")
        lines.append(f"Optimization dir: {summary.get('optimization_dir', '')}")
        lines.append(
            f"FEM option validation dir: {summary.get('fem_option_validation_dir', summary.get('option_microbench_dir', ''))}"
        )
        lines.append(f"Backend: {summary.get('backend', '')}")
        lines.append(f"Execution mode: {summary.get('execution_mode', '')}")
        if summary.get("json_path"):
            lines.append(f"JSON: {summary.get('json_path', '')}")
        if summary.get("markdown_path"):
            lines.append(f"Markdown: {summary.get('markdown_path', '')}")
        if summary.get("option_alignment_csv"):
            lines.append(f"Option alignment CSV: {summary.get('option_alignment_csv', '')}")
        if summary.get("profile_proximity_csv"):
            lines.append(f"Profile proximity CSV: {summary.get('profile_proximity_csv', '')}")
        if summary.get("category_summary_csv"):
            lines.append(f"Category summary CSV: {summary.get('category_summary_csv', '')}")
        plots = summary.get("plots") or {}
        if isinstance(plots, dict) and plots:
        ...
        if best:
            ...
            lines.append(f"Option index: {best.get('option_index', '')}")
            lines.append(f"Combo bits: {best.get('combo_bits', '')}")
        alignment = summary.get("option_alignment") or []
        if alignment:
            lines.append("")
            lines.append("Option alignment")
            for row in alignment:
                if not isinstance(row, dict):
                    continue
                lines.append(
                    f"{row.get('probe_id')}: enabled={row.get('best_config_enabled')} "
                    f"mean_delta_ratio={row.get('mean_delta_ratio')} "
                    f"supports_best_config={row.get('supports_best_config')}"
                )
        profiler_reports = summary.get("profiler_reports") or []
        if profiler_reports:
            lines.append("")
            lines.append("Profiler reports")
            for report in profiler_reports:
                if not isinstance(report, dict):
                    continue
                lines.append(f"{report.get('kind')}: {report.get('path')}")
        if summary.get("summary_hash"):
            lines.append("")
            lines.append(f"summary_hash: {summary.get('summary_hash')}")
        return "\n".join(lines)

    def _format_probe_support_panel(self, payload: Dict[str, Any]) -> str:
        lines: List[str] = []
        lines.append("Probe support for best exact/native configuration")
        lines.append("")
        best = payload.get("best_overall") or {}
        if best:
            lines.append(f"Best variant: {best.get('variant', '')}")
            lines.append(f"Best option index: {best.get('option_index', '')}")
            if best.get("combo_bits"):
                lines.append(f"Combo bits: {best.get('combo_bits')}")
        alignment = payload.get("option_alignment") or []
        if alignment:
            lines.append("")
            lines.append("Single-control probes")
            for row in alignment:
                if not isinstance(row, dict):
                    continue
                lines.append(
                    f"{row.get('probe_label', row.get('probe_id'))}: "
                    f"enabled={row.get('best_config_enabled')} "
                    f"recommended={row.get('recommended_state')} "
                    f"mean_delta_ratio={row.get('mean_delta_ratio')} "
                    f"supports={row.get('supports_best_config')}"
                )
                rationale = str(row.get("rationale", "")).strip()
                if rationale:
                    lines.append(f"  rationale: {rationale}")
        profile_proximity = payload.get("profile_proximity") or []
        if profile_proximity:
            lines.append("")
            lines.append("Composite profiles")
            for row in profile_proximity:
                if not isinstance(row, dict):
                    continue
                lines.append(
                    f"{row.get('probe_label', row.get('probe_id'))}: "
                    f"closer_to={row.get('closer_to')} "
                    f"baseline_distance={row.get('baseline_distance')} "
                    f"toggled_distance={row.get('toggled_distance')}"
                )
        ...
        source = str(payload.get("_probe_support_source", "")).strip()
        if source:
            lines.append("")
            lines.append(f"source: {source}")
        return "\n".join(lines)

    def _set_probe_support_text(self, text: str) -> None:
        self.probe_support_text.delete("1.0", "end")
        self.probe_support_text.insert("end", text)

    def _probe_support_from_correlation_summary(self, summary: Dict[str, Any], *, source: str = "") -> Dict[str, Any]:
        payload = dict(summary)
        if source:
            payload["_probe_support_source"] = source
        return payload

    def _probe_support_from_exact_summary(self, summary: Dict[str, Any], validation_dir: Path) -> Dict[str, Any] | None:
        best = dict(summary.get("best_overall") or {})
        if not best:
            return None
        ...
        probe_summary = dict(validation_summary.get("probe_summary") or {})
        probe_catalog = dict(validation_summary.get("probe_catalog") or {})
        payload = {
            "best_overall": best,
            "option_alignment": compute_option_alignment(best, probe_summary, probe_catalog),
            "profile_proximity": compute_profile_proximity(best, probe_catalog),
            "category_summary": validation_summary.get("category_summary") or {},
            "_probe_support_source": str(validation_summary_path),
        }
        return payload

    def _refresh_probe_support_panel(self, payload: Any) -> None:
        summary = payload.get("summary") if isinstance(payload, dict) and isinstance(payload.get("summary"), dict) else payload
        if not isinstance(summary, dict):
            self._set_probe_support_text(
                "Load a profiler-correlation result, or load an exact/native run together with a FEM option validation result, to see probe support here."
            )
            return
        workflow = str(summary.get("workflow", "")).strip()
        if workflow == "profiler_correlation":
            self._set_probe_support_text(self._format_probe_support_panel(self._probe_support_from_correlation_summary(summary)))
            return

        base_dir = self.last_result_dir if isinstance(self.last_result_dir, Path) else None
        if base_dir is not None:
            correlation_summary = base_dir / "profiler_correlation" / "profiler_correlation.json"
            if correlation_summary.exists():
                try:
                    corr = _read_json(correlation_summary)
                    self._set_probe_support_text(
                        self._format_probe_support_panel(self._probe_support_from_correlation_summary(corr, source=str(correlation_summary)))
                    )
                    return
                except Exception as exc:
                    self._set_probe_support_text(f"Probe support could not be loaded from {correlation_summary}: {exc}")
                    return

        validation_dir_raw = self.workflow_correlation_fem_option_validation_dir_var.get().strip()
        if validation_dir_raw:
            validation_dir = Path(validation_dir_raw).expanduser().resolve()
            if validation_dir.exists():
                try:
                    support = self._probe_support_from_exact_summary(summary, validation_dir)
                    if support is not None:
                        self._set_probe_support_text(self._format_probe_support_panel(support))
                        return
                except Exception as exc:
                    self._set_probe_support_text(f"Probe support could not be derived from {validation_dir}: {exc}")
                    return

        self._set_probe_support_text(
            "No probe-support payload found yet. Load a profiler-correlation result, or pick a FEM option validation directory and then load an exact/native result."
        )
    ...
    def _format_results_payload(self, payload: Any) -> str:
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(summary, dict) and str(summary.get("workflow", "")).strip() == "fem_option_validation":
                return self._format_fem_option_validation_payload(summary)
            if isinstance(summary, dict) and str(summary.get("workflow", "")).strip() == "profiler_correlation":
                return self._format_profiler_correlation_payload(summary)
    ...
    def _set_results_text(self, payload: Any) -> None:
        ...
        self._refresh_probe_support_panel(payload)
    ...
    def _workflow_command(self) -> tuple[str, str]:
        ...
        if bool(self.workflow_filip_export_expected_output_var.get()):
            ...
            if "--filip-dump-launch-artifacts" not in args:
                args.append("--filip-dump-launch-artifacts")
        if bool(self.workflow_filip_export_canonical_bundles_var.get()):
            args.append("--filip-export-canonical-replay-bundles")
            if "--filip-export-replay-inputs" not in args:
                args.append("--filip-export-replay-inputs")
            if "--filip-dump-launch-artifacts" not in args:
                args.append("--filip-dump-launch-artifacts")
        ...
        args.extend(
            [
                ...
                self.workflow_fem_option_validation_ops_var.get().strip() or "laplace,test",
                "--fem-option-validation-variants",
                self.workflow_fem_option_validation_variants_var.get().strip() or "qss,sqs,ssq",
                "--fem-option-validation-n-elements",
                self.workflow_fem_option_validation_n_elements_var.get().strip() or "4096",
                "--fem-option-validation-n-qp",
                self.workflow_fem_option_validation_n_qp_var.get().strip() or "6",
                "--fem-option-validation-workgroup-size",
                self.workflow_fem_option_validation_workgroup_var.get().strip() or "64",
            ]
        )
        correlation_optimization_dir = self.workflow_correlation_optimization_dir_var.get().strip()
        if correlation_optimization_dir:
            args.extend(["--correlation-optimization-dir", correlation_optimization_dir])
        correlation_option_dir = self.workflow_correlation_fem_option_validation_dir_var.get().strip()
        if correlation_option_dir:
            args.extend(["--correlation-fem-option-validation-dir", correlation_option_dir])
        for report in [item.strip() for item in self.workflow_correlation_profiler_reports_var.get().split("|") if item.strip()]:
            args.extend(["--correlation-profiler-report", report])
    ...
    def _refresh_workflow_description(self) -> None:
        ...
        if spec.get("id") == "filip_original":
            ...
            if filip_mode == "exact_reference":
                ...
                if self.workflow_filip_export_canonical_bundles_var.get():
                    exact_bits.append("canonical replay bundles export enabled")
                if exact_bits:
                    desc = f"{desc} Exact extras: {'; '.join(exact_bits)}."
        elif spec.get("id") == "fem_option_validation":
            desc = (
                f"{desc} Operators: {self.workflow_fem_option_validation_ops_var.get().strip() or 'laplace,test'}. "
                f"Variants: {self.workflow_fem_option_validation_variants_var.get().strip() or 'qss,sqs,ssq'}. "
                f"Size: n_elements={self.workflow_fem_option_validation_n_elements_var.get().strip() or '4096'}, "
                f"n_qp={self.workflow_fem_option_validation_n_qp_var.get().strip() or '6'}, "
                f"workgroup={self.workflow_fem_option_validation_workgroup_var.get().strip() or '64'}."
            )
        elif spec.get("id") == "profiler_correlation":
            reports = [item.strip() for item in self.workflow_correlation_profiler_reports_var.get().split("|") if item.strip()]
            desc = (
                f"{desc} Optimization dir: {self.workflow_correlation_optimization_dir_var.get().strip() or '(not set)'}. "
                f"FEM option validation dir: {self.workflow_correlation_fem_option_validation_dir_var.get().strip() or '(not set)'}. "
                f"Profiler reports: {len(reports)}."
            )
    ...
    def _browse_workflow_correlation_optimization_dir(self) -> None:
        initial_dir = self.workflow_correlation_optimization_dir_var.get().strip() or str(ROOT / "data" / "optimization")
        selected = filedialog.askdirectory(initialdir=initial_dir, title="Select optimization/exact result directory")
        if selected:
            self.workflow_correlation_optimization_dir_var.set(selected)

    def _browse_workflow_correlation_fem_option_validation_dir(self) -> None:
        initial_dir = self.workflow_correlation_fem_option_validation_dir_var.get().strip() or str(ROOT / "data" / "fem_option_validation")
        selected = filedialog.askdirectory(initialdir=initial_dir, title="Select FEM option validation result directory")
        if selected:
            self.workflow_correlation_fem_option_validation_dir_var.set(selected)

    def _browse_workflow_correlation_profiler_reports(self) -> None:
        initial_dir = str(ROOT)
        selected = filedialog.askopenfilenames(
            initialdir=initial_dir,
            title="Select profiler report exports",
            filetypes=[("Profiler exports", "*.json *.csv"), ("All files", "*.*")],
        )
        if selected:
            self.workflow_correlation_profiler_reports_var.set("|".join(selected))
    ...
    def _workflow_discovery_backends(self) -> List[str]:
        ...
        if workflow_id == "profiler_correlation":
            return ["cpu"]
    ...
    def _on_process_done(self, exit_code: int) -> None:
        ...
        if out_dir is not None and out_dir.exists():
            ...
            if _script_match(script, "run_workflow.py"):
                ...
                self._load_optimization_dir(out_dir)
            elif _script_match(script, "run_fem_option_validation.py") or _script_match(script, "scripts/run_profiler_correlation.py"):
                self._load_result_dir(out_dir)
            else:
    ...
    def _load_selected_result_dir(self) -> None:
        path = Path(self.result_dir_var.get().strip())
        self._load_result_dir(path)
    ...
    def _load_result_dir(self, path: Path) -> None:
        if not path.exists():
            messagebox.showerror("Missing result", f"Path does not exist: {path}")
            return
        if path.is_file():
            if path.suffix.lower() in {".md", ".txt", ".json"}:
                self._show_text_file(path)
                return
            messagebox.showerror("Unsupported file", f"Cannot load result file directly: {path}")
            return

        profiler_json = path / "profiler_correlation.json"
        summary_path = path / "summary.json"
        manifest_path = path / "manifest.json"

        if profiler_json.exists() and not summary_path.exists():
            self._load_profiler_correlation_dir(path)
            return
        if manifest_path.exists():
            self._load_session_dir(path)
            return
        if not summary_path.exists():
            messagebox.showerror("Invalid result directory", f"Missing summary.json or profiler_correlation.json in {path}")
            return
        ...
        workflow = str(summary.get("workflow", "")).strip()
        if workflow == "fem_option_validation":
            self._load_fem_option_validation_dir(path)
            return
        if workflow == "profiler_correlation":
            self._load_profiler_correlation_dir(path)
            return
        if "comparisons_vs_baseline" in summary:
            self._load_matrix_dir(path)
            return
        ...
        self.latest_result_payload = {"summary": summary, "out_dir": str(path)}
        self.latest_optimization_data = {}
        self.latest_matrix_data = {}
        self.latest_fem_validation_data = {}
        self.latest_profiler_correlation_data = {}
        self.last_result_dir = path
        self.result_dir_var.set(str(path))
        self._set_results_text(self.latest_result_payload)
        self.status_var.set(f"Loaded result summary: {path.name}")

    def _load_fem_option_validation_dir(self, path: Path) -> None:
        summary_path = path / "summary.json"
        if not summary_path.exists():
            messagebox.showerror("Invalid validation directory", f"Missing file: {summary_path}")
            return
        try:
            summary = _read_json(summary_path)
        ...
        self.latest_result_payload = dict(self.latest_fem_validation_data)
        self.latest_optimization_data = {}
        self.latest_matrix_data = {}
        self.latest_profiler_correlation_data = {}
        self.last_result_dir = path
        self.result_dir_var.set(str(path))
        self._set_results_text(self.latest_fem_validation_data)
        self._plot_fem_option_validation()
        self.status_var.set(f"Loaded FEM option validation: {path.name}")

    def _load_profiler_correlation_dir(self, path: Path) -> None:
        summary_path = path / "summary.json"
        fallback_path = path / "profiler_correlation.json"
        target_path = summary_path if summary_path.exists() else fallback_path
        if not target_path.exists():
            messagebox.showerror("Invalid correlation directory", f"Missing file: {summary_path} or {fallback_path}")
            return
        try:
            summary = _read_json(target_path)
        except Exception as exc:
            messagebox.showerror("Load failed", str(exc))
            return
        ...
        self.latest_result_payload = dict(self.latest_profiler_correlation_data)
        self.latest_optimization_data = {}
        self.latest_matrix_data = {}
        self.latest_fem_validation_data = {}
        self.last_result_dir = path
        self.result_dir_var.set(str(path))
        self._set_results_text(self.latest_profiler_correlation_data)
        self._plot_profiler_correlation()
        self.status_var.set(f"Loaded profiler correlation: {path.name}")
    ...
    def _load_latest_fem_option_validation(self) -> None:
        base = ROOT / "data" / "fem_option_validation"
        if not base.exists():
            messagebox.showwarning("No results", f"No directory: {base}")
            return
        dirs = [d for d in base.iterdir() if d.is_dir() and (d / "summary.json").exists()]
        if not dirs:
            messagebox.showwarning("No results", "No FEM option validation runs found.")
            return
        latest = max(dirs, key=lambda d: d.stat().st_mtime)
        self.result_dir_var.set(str(latest))
        self._load_fem_option_validation_dir(latest)

    def _load_latest_profiler_correlation(self) -> None:
        base = ROOT / "data" / "optimization"
        if not base.exists():
            messagebox.showwarning("No results", f"No directory: {base}")
            return
        ...
        for parent in base.iterdir():
            if not parent.is_dir():
                continue
            candidate = parent / "profiler_correlation"
            if (candidate / "summary.json").exists() or (candidate / "profiler_correlation.json").exists():
                candidates.append(candidate)
        if not candidates:
            messagebox.showwarning("No results", "No profiler correlation runs found.")
            return
        latest = max(candidates, key=lambda d: d.stat().st_mtime)
        self.result_dir_var.set(str(latest))
        self._load_profiler_correlation_dir(latest)

    def _load_latest_thesis_full(self) -> None:
        ...
        dirs = [d for d in base.iterdir() if d.is_dir() and (d / "summary.json").exists()]
        if not dirs:
            messagebox.showwarning("No results", "No full thesis campaigns found.")
            return
        latest = max(dirs, key=lambda d: d.stat().st_mtime)
        self.result_dir_var.set(str(latest))
        self._load_result_dir(latest)
    ...
    def _optimization_plots_dir(base_dir: Path) -> Path | None:
        candidates = [
            base_dir / "figures" / "thesis_core",
            base_dir / "figures" / "appendix",
            base_dir / "plots",
            ROOT / "analysis" / "plots" / f"{base_dir.name}__filip_article",
        ]
    ...
    @staticmethod
    def _resolve_plot_path(raw: Any, *, base_dir: Path | None = None) -> Path | None:
        text = str(raw).strip()
        if not text:
            return None
        p = Path(text)
    ...
    @staticmethod
    def _summary_plot_paths(summary: Dict[str, Any], *, base_dir: Path | None = None) -> List[Path]:
        plots = summary.get("plots") or {}
        paths: List[Path] = []
        if isinstance(plots, dict):
            for raw in plots.values():
                p = AutotuneGui._resolve_plot_path(raw, base_dir=base_dir)
                if p is not None:
                    paths.append(p)
        figure_set_plots = summary.get("figure_set_plots") or []
        if isinstance(figure_set_plots, list):
            for raw in figure_set_plots:
                p = AutotuneGui._resolve_plot_path(raw, base_dir=base_dir)
                if p is not None:
                    paths.append(p)
        figure_appendix_plots = summary.get("figure_appendix_plots") or []
        if isinstance(figure_appendix_plots, list):
            for raw in figure_appendix_plots:
                p = AutotuneGui._resolve_plot_path(raw, base_dir=base_dir)
                if p is not None:
                    paths.append(p)
        article_plots = summary.get("article_plots") or []
        if isinstance(article_plots, list):
            for raw in article_plots:
                p = AutotuneGui._resolve_plot_path(raw, base_dir=base_dir)
                if p is not None:
                    paths.append(p)
        figure_set_dir = summary.get("figure_set_dir")
        if figure_set_dir:
            pdir = Path(str(figure_set_dir))
            if not pdir.is_absolute() and base_dir is not None:
                pdir = base_dir / pdir
            selected = AutotuneGui._preferred_plot_file(pdir)
            if selected is not None:
                paths.append(selected)
        article_plots_dir = summary.get("article_plots_dir")
        if article_plots_dir:
            pdir = Path(str(article_plots_dir))
            if not pdir.is_absolute() and base_dir is not None:
                pdir = base_dir / pdir
            selected = AutotuneGui._preferred_plot_file(pdir)
            if selected is not None:
    ...
    @staticmethod
    def _preferred_named_summary_plot(summary: Dict[str, Any], names: List[str], *, base_dir: Path | None = None) -> Path | None:
        plots = summary.get("plots") or {}
        if isinstance(plots, dict):
            for name in names:
                if name in plots:
                    p = AutotuneGui._resolve_plot_path(plots.get(name), base_dir=base_dir)
                    if p is not None:
                        return p
        paths = AutotuneGui._summary_plot_paths(summary, base_dir=base_dir)
    ...
    def _load_session_dir(self, path: Path) -> None:
        ...
        self.last_compare_dir = None
        manifest = path / "manifest.json"
        if manifest.exists():
            try:
                payload = _read_json(manifest)
                payload["_session_dir"] = str(path)
                self.latest_result_payload = payload
                self.latest_optimization_data = {}
                self.latest_matrix_data = {}
                self.latest_fem_validation_data = {}
                self.latest_profiler_correlation_data = {}
                self._set_results_text(payload)
                self._plot_loaded_result()
            except Exception:
    ...
    def _current_session_dir_from_payload(self) -> Path | None:
        payload = self.latest_result_payload or {}
        if not isinstance(payload, dict):
            return None
        raw = str(payload.get("_session_dir", "")).strip()
        if raw:
            path = Path(raw)
            if path.exists():
                return path
        current = self.result_dir_var.get().strip()
        if current:
            path = Path(current)
    ...
    @staticmethod
    def _session_csv_rows(path: Path) -> List[Dict[str, str]]:
        try:
    ...
    @staticmethod
    def _session_float(value: Any) -> float | None:
        try:
            if value is None:
                return None
    ...
    def _plot_session_dashboard(self, session_dir: Path) -> None:
        if not HAS_MPL or self.figure is None or self.canvas is None:
            return

        cpu_dir = session_dir / "cpu"
        gpu_dir = session_dir / "gpu"
        roofline_json = session_dir / "roofline" / "roofline_result.json"
        workflow = str((self.latest_result_payload or {}).get("workflow", "")).strip() or "session"

        self.figure.clear()
        axes = self.figure.subplots(2, 2)
        ax_bw, ax_lat = axes[0]
        ax_compute, ax_roof = axes[1]

        def plot_curves(ax, *, patterns: List[str], x_keys: List[str], y_keys: List[str], title: str, xlabel: str, ylabel: str) -> bool:
            plotted = False
            cache_annotation_rows: List[Dict[str, Any]] = []
            for scope, base in (("CPU", cpu_dir), ("GPU", gpu_dir)):
                if not base.exists():
                    continue
                for pattern in patterns:
                    for csv_path in sorted(base.glob(pattern)):
                        rows = self._session_csv_rows(csv_path)
                        grouped: Dict[float, List[float]] = {}
                        label_name = csv_path.stem
                        for row in rows:
                            x_val: float | None = None
                            for key in x_keys:
                                x_val = self._session_float(row.get(key))
                            ...
                            for key in y_keys:
                                y_val = self._session_float(row.get(key))
                                if y_val is not None:
                                    break
                            if x_val is None or y_val is None or x_val <= 0.0:
                                continue
                            grouped.setdefault(float(x_val), []).append(float(y_val))
                            label_name = str(row.get("benchmark") or row.get("kernel") or label_name)
                        if not grouped:
                            continue
                        xs = sorted(grouped.keys())
                        ys = [sum(grouped[x]) / len(grouped[x]) for x in xs]
                        ax.plot(xs, ys, marker="o", linewidth=1.4, label=f"{scope}: {label_name}")
                        if pattern == "*pointer_latency*.csv" and scope == "CPU":
                            cache_annotation_rows.extend(rows)
                        plotted = True
            if plotted:
                ax.set_title(title)
                ...
                try:
                    ...
                    pass
                if pattern == "*pointer_latency*.csv" and cache_annotation_rows:
                    add_cache_boundary_lines(ax, cache_annotation_rows)
            else:
        ...
        plot_curves(
            ax_bw,
            patterns=["*bandwidth*.csv"],
            x_keys=["size_bytes", "size_mb"],
            y_keys=["gbps", "throughput_gbps"],
            title="Bandwidth vs size",
            xlabel="Working-set size [bytes]",
            ylabel="GB/s",
        )
        plot_curves(
            ax_lat,
            patterns=["*pointer_latency*.csv", "*tlb_latency*.csv"],
            x_keys=["working_set_bytes", "size_bytes"],
            y_keys=["latency_ns"],
            title="Memory / TLB latency",
            xlabel="Working-set size [bytes]",
            ylabel="ns/access",
        )

        compute_labels: List[str] = []
        compute_vals: List[float] = []
        for scope, base in (("CPU", cpu_dir), ("GPU", gpu_dir)):
            if not base.exists():
                continue
            for csv_path in sorted(list(base.glob("*compute*.csv")) + list(base.glob("stream*.csv"))):
                rows = self._session_csv_rows(csv_path)
                best_value: float | None = None
                label_name = csv_path.stem
                for row in rows:
                    value = self._session_float(row.get("gflops"))
                    if value is None:
                        value = self._session_float(row.get("gbps"))
                    if value is None:
                        continue
                    best_value = value if best_value is None else max(best_value, value)
                    label_name = str(row.get("benchmark") or row.get("kernel") or label_name)
                if best_value is not None:
                    compute_labels.append(f"{scope}: {label_name}")
                    compute_vals.append(best_value)
        if compute_vals:
            xs = list(range(len(compute_labels)))
            ax_compute.bar(xs, compute_vals, color="#2563eb", alpha=0.9)
            ax_compute.set_xticks(xs)
            ax_compute.set_xticklabels(compute_labels, rotation=25, ha="right", fontsize=8)
            ax_compute.set_ylabel("Best observed throughput")
            ax_compute.set_title("Compute / stream summary")
            ax_compute.grid(True, axis="y", alpha=0.25)
        else:
            ax_compute.text(0.5, 0.5, "No compute/stream data", transform=ax_compute.transAxes, ha="center", va="center")
            ax_compute.set_axis_off()

        roof_plotted = False
        if roofline_json.exists():
            try:
                roof = _read_json(roofline_json)
                for item in roof.get("results") or []:
                    record = item.get("record") or {}
                    estimate = item.get("estimate") or {}
                    peak_bw = self._session_float(record.get("peak_bw_gbps"))
                    peak_gflops = self._session_float(record.get("peak_gflops"))
                    ai = self._session_float(estimate.get("ai_flop_per_byte"))
                    attainable = self._session_float(estimate.get("attainable_gflops"))
                    if peak_bw is None or peak_gflops is None or ai is None or attainable is None:
                        continue
                    knee = peak_gflops / peak_bw if peak_bw > 0 else 1.0
                    x_max = max(ai * 2.0, knee * 4.0, 16.0)
                    xs = [2 ** (i / 4.0 - 4.0) for i in range(0, int(max(24, math.log2(x_max) * 4 + 20)))]
                    ys = [min(peak_gflops, peak_bw * x) for x in xs]
                    label = str(record.get("model") or record.get("backend") or "roofline")
                    ax_roof.plot(xs, ys, linewidth=1.5, label=label)
                    ax_roof.scatter([ai], [attainable], color="#dc2626", zorder=4)
                    roof_plotted = True
                if roof_plotted:
                    ax_roof.set_xscale("log", base=2)
                    ax_roof.set_xlabel("Arithmetic intensity [FLOP/byte]")
                    ax_roof.set_ylabel("Attainable GFLOP/s")
                    ax_roof.set_title("Roofline estimate")
                    ax_roof.grid(True, which="both", alpha=0.25)
                    ax_roof.legend(fontsize=7)
            except Exception:
                roof_plotted = False
        if not roof_plotted:
            ax_roof.text(0.5, 0.5, "No roofline data", transform=ax_roof.transAxes, ha="center", va="center")
            ax_roof.set_axis_off()

        self.figure.suptitle(f"Session dashboard: {workflow}", fontsize=14)
        self.figure.tight_layout(rect=[0, 0.02, 1, 0.97])
        self.canvas.draw_idle()
        self.status_var.set(f"Rendered session dashboard: {session_dir.name}")

    def _plot_loaded_result(self) -> None:
        summary: Dict[str, Any] = {}
        payload = self.latest_result_payload or {}
        if isinstance(payload, dict):
            if isinstance(payload.get("summary"), dict):
                ...
                summary = payload
        summary_plot_paths = self._summary_plot_paths(summary, base_dir=self.last_result_dir) if isinstance(summary, dict) else []
        if summary_plot_paths:
            self._plot_image_file(summary_plot_paths[0])
            return
        session_dir = self._current_session_dir_from_payload()
        if session_dir is not None:
            self._plot_session_dashboard(session_dir)
            return
        workflow = str(summary.get("workflow", "")).strip()
        if workflow == "fem_option_validation":
            self._plot_fem_option_validation()
        ...
        if "comparisons_vs_baseline" in summary:
            ...
            return
        if self.latest_optimization_data:
            if self.last_result_dir is not None and self._optimization_plots_dir(self.last_result_dir) is not None:
                self._show_current_optimization_plot()
            else:
                self._plot_firefly_convergence()
            return
        messagebox.showwarning("No plot", "Load a result directory first.")
    ...
    def _plot_fem_option_validation(self) -> None:
        if not HAS_MPL or self.figure is None or self.canvas is None:
            return
        data = self.latest_fem_validation_data or {}
        summary = data.get("summary") or {}
        if not summary:
            messagebox.showwarning("No validation result", "Load a FEM option validation result first.")
            return
        preferred = self._preferred_named_summary_plot(summary, ["probe_plot", "matrix_plot"], base_dir=self.last_result_dir)
        if preferred is not None:
            self._plot_image_file(preferred)
            return
        self.figure.clear()
        ...
        for probe_id, values in probe_summary.items():
            ...
            try:
                ...
                ratio = float("nan")
            label = str(values.get("probe_label", probe_id))
            probe_rows.append((label, ratio))
        ...
        if probe_rows:
            ...
            ax1.barh(ys, ratios, color=colors, alpha=0.9)
            ax1.axvline(1.0, color="#334155", linestyle="--", linewidth=1.0)
            ax1.set_yticks(ys)
            ax1.set_yticklabels(labels, fontsize=8)
            ax1.set_xlabel("Mean delta ratio (toggled / baseline)")
            ax1.set_title("FEM option validation probes")
            ax1.grid(True, axis="x", alpha=0.25)
        else:
            ax1.text(0.5, 0.5, "No probe summary data", ha="center", va="center")
            ax1.set_axis_off()

        records = data.get("records") or []
        if records:
            probe_ids = list((summary.get("probe_catalog") or {}).keys()) or sorted(
            ...
            if probe_ids and combo_keys:
                ...
                flattened = [val for row in matrix for val in row if math.isfinite(val)]
                if flattened:
                    vmax = max(max(flattened), 1.0)
                    vmin = min(min(flattened), 1.0)
                    img = ax2.imshow(matrix, aspect="auto", cmap="coolwarm", vmin=vmin, vmax=vmax)
                    self.figure.colorbar(img, ax=ax2, label="Mean delta ratio")
                    ax2.set_xticks(list(range(len(probe_ids))))
                    ax2.set_xticklabels(
                        [str((summary.get("probe_catalog") or {}).get(pid, {}).get("label", pid)) for pid in probe_ids],
                        rotation=35,
                        ha="right",
                        fontsize=8,
                    )
                    ax2.set_yticks(list(range(len(combo_keys))))
                    ax2.set_yticklabels(combo_keys, fontsize=8)
                    ax2.set_title("Probe sensitivity by operator and variant")
                else:
                    ax2.text(0.5, 0.5, "No finite record matrix values", ha="center", va="center")
                    ax2.set_axis_off()
            else:
                ax2.text(0.5, 0.5, "Insufficient operator/variant records", ha="center", va="center")
                ax2.set_axis_off()
        else:
            category_summary = summary.get("category_summary") or {}
            if category_summary:
                ...
                vals = [
                    float((category_summary.get(cat) or {}).get("mean_delta_ratio", float("nan")))
                    for cat in categories
                ]
                colors = ["#0f766e" if math.isfinite(v) and v < 1.0 else "#7c2d12" for v in vals]
                ax2.bar(categories, vals, color=colors, alpha=0.9)
                ax2.axhline(1.0, color="#334155", linestyle="--", linewidth=1.0)
                ax2.set_ylabel("Mean delta ratio")
                ax2.set_title("Category summary")
                ax2.tick_params(axis="x", rotation=20)
                ax2.grid(True, axis="y", alpha=0.25)
            else:
                ax2.text(0.5, 0.5, "No category summary data", ha="center", va="center")
                ax2.set_axis_off()

        self.figure.tight_layout()
        self.canvas.draw_idle()
        self.status_var.set("Rendered FEM option validation plot")

    def _plot_profiler_correlation(self) -> None:
        if not HAS_MPL or self.figure is None or self.canvas is None:
            return
        data = self.latest_profiler_correlation_data or {}
        summary = data.get("summary") or {}
        if not summary:
            messagebox.showwarning("No correlation result", "Load a profiler correlation result first.")
            return
        preferred = self._preferred_named_summary_plot(
            summary,
            ["option_alignment_plot", "profile_proximity_plot", "category_summary_plot"],
            base_dir=self.last_result_dir,
        )
        if preferred is not None:
            self._plot_image_file(preferred)
        ...
        alignment = [row for row in (summary.get("option_alignment") or []) if isinstance(row, dict)]
        if alignment:
            ...
            ax1.bar(xs, ratios, color=colors, alpha=0.9)
            ax1.axhline(1.0, color="#334155", linestyle="--", linewidth=1.0)
            ax1.set_xticks(xs)
            ax1.set_xticklabels(labels, rotation=35, ha="right", fontsize=8)
            ax1.set_ylabel("Mean delta ratio")
            ax1.set_title("Probe alignment with best exact/native configuration")
            ax1.grid(True, axis="y", alpha=0.25)
        else:
            ax1.text(0.5, 0.5, "No option alignment data", ha="center", va="center")
            ax1.set_axis_off()

        profiles = [row for row in (summary.get("profile_proximity") or []) if isinstance(row, dict)]
        if profiles:
            labels = [str(row.get("probe_label", row.get("probe_id", ""))) for row in profiles]
            baseline = [float(row.get("baseline_distance", float("nan"))) for row in profiles]
            toggled = [float(row.get("toggled_distance", float("nan"))) for row in profiles]
            xs = list(range(len(labels)))
            width = 0.38
            ax2.bar([x - width / 2 for x in xs], baseline, width=width, label="distance to baseline", color="#475569")
            ax2.bar([x + width / 2 for x in xs], toggled, width=width, label="distance to toggled", color="#2563eb")
            ax2.set_xticks(xs)
            ax2.set_xticklabels(labels, rotation=35, ha="right", fontsize=8)
            ax2.set_ylabel("Hamming distance")
            ax2.set_title("Composite-profile proximity")
            ax2.grid(True, axis="y", alpha=0.25)
            ax2.legend()
        else:
            category_summary = summary.get("category_summary") or {}
            if category_summary:
                categories = sorted(category_summary.keys())
                vals = [float((category_summary.get(cat) or {}).get("mean_delta_ratio", float("nan"))) for cat in categories]
                ax2.bar(categories, vals, color="#0f766e", alpha=0.9)
                ax2.axhline(1.0, color="#334155", linestyle="--", linewidth=1.0)
                ax2.set_ylabel("Mean delta ratio")
                ax2.set_title("Correlation category summary")
                ax2.tick_params(axis="x", rotation=20)
                ax2.grid(True, axis="y", alpha=0.25)
            else:
                ax2.text(0.5, 0.5, "No profile proximity data", ha="center", va="center")
                ax2.set_axis_off()

        self.figure.tight_layout()
        self.canvas.draw_idle()
        self.status_var.set("Rendered profiler correlation plot")
    ...
    def _load_compare_dir(self, path: Path) -> None:
        ...
        self.latest_result_payload = dict(summary)
        self.latest_optimization_data = {}
        self.latest_matrix_data = {}
        self.latest_fem_validation_data = {}
        self.latest_profiler_correlation_data = {}
    ...
    def _load_optimization_dir(self, path: Path) -> None:
        ...
        self.latest_result_payload = dict(self.latest_optimization_data)
        self.latest_matrix_data = {}
        self.latest_fem_validation_data = {}
        self.latest_profiler_correlation_data = {}
        self.last_result_dir = path
    ...
    def _plot_firefly_convergence(self) -> None:
        ...
        preferred = self._preferred_named_summary_plot(
            summary,
            ["convergence_plot", "primary_metric_plot", "scatter_plot"],
            base_dir=self.last_result_dir,
        )
        if preferred is not None:
            self._plot_image_file(preferred)
    ...
    def _plot_firefly_scatter(self) -> None:
        ...
        preferred = self._preferred_named_summary_plot(
            summary,
            ["scatter_plot", "primary_metric_plot", "convergence_plot"],
            base_dir=self.last_result_dir,
        )
        if preferred is not None:
            self._plot_image_file(preferred)
