...
def _run_backend(
    ...
    print(f"runs  : {runs}, warmups: {warmups}, n_qp: {n_qp}, operator: {operator}")

    for n_elem in sizes:
        print(f"\n--- n_elements: {n_elem} ---")
        if element_type == "tet4":
            run_warmups(warmups, lambda: be.fem_integration_tet4(n_elements=n_elem, n_qp=n_qp, operator=operator, dtype=dtype))
        elif element_type == "hex8":
            run_warmups(warmups, lambda: be.fem_integration_hex8(n_elements=n_elem, n_qp=n_qp, operator=operator, dtype=dtype))
        elif element_type == "prism6":
            run_warmups(warmups, lambda: be.fem_integration_prism6(n_elements=n_elem, n_qp=n_qp, operator=operator, dtype=dtype))
