...
def _run_backend(backend: str, device_index: int, sizes: List[int], n_qp: int, runs: int, warmups: int, dtype: str) -> None:
    if backend == "cpu":
        be = CpuRealBackend()
    ...
    print(f"runs  : {runs}, warmups: {warmups}, n_qp: {n_qp}")

    for n_elem in sizes:
        print(f"\n--- n_elements: {n_elem} ---")
        run_warmups(warmups, lambda: be.fem_element(n_elements=n_elem, n_qp=n_qp, dtype=dtype))
...
def main() -> None:
    ...
    for b in backends:
        try:
            _run_backend(b, args.device_index, sizes, args.n_qp, args.runs, args.warmups, args.dtype)
        except Exception as e:
