...
class FireflyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.elite_keep < 0:
            raise ValueError("elite_keep must be >= 0")
        if self.elite_keep > self.population_size:
            raise ValueError("elite_keep cannot exceed population_size")
...
class FireflyOptimizer:
    ...
    def __init__(
        ...
        self.archive: List[FireflyAgent] = []
        self._archive_by_signature: Dict[str, FireflyAgent] = {}
    ...
    def _initialize(self) -> None:
        self.population = []
        self.archive = []
        self._archive_by_signature = {}
        occupied: set[str] = set()
        for i in range(self.config.population_size):
            pos = self._unique_position(occupied)
            occupied.add(self._position_signature(pos))
    ...
    def _evaluate_population(self, iteration: int) -> None:
        ...
        for ff in self.population:
            ...
            if sig not in self._archive_by_signature:
                archived = FireflyAgent(
                    idx=ff.idx,
                    position=list(ff.position),
                    config=dict(cfg),
                    result=res,
                    brightness=float("-inf"),
                )
                self.archive.append(archived)
                self._archive_by_signature[sig] = archived

        self._refresh_archive_brightness()

        for ff, sig in zip(self.population, current_signatures):
            archived = self._archive_by_signature[sig]
            ff.brightness = float(archived.brightness)
            row: Dict[str, Any] = {
    ...
    def _scored_item(self, ff: FireflyAgent) -> ScoredItem:
        res = ff.result
        if res is None:
            return ScoredItem(feasible=False, metrics={}, violations=["missing_result"])
        return ScoredItem(
            feasible=res.constraints_ok and res.status == "ok",
            metrics=dict(res.metrics),
            violations=list(res.violations),
        )

    def _refresh_archive_brightness(self) -> None:
        if not self.archive:
            return
        scored = [self._scored_item(ff) for ff in self.archive]
        brightness = self.objective_model.brightness(scored)
        for ff, b in zip(self.archive, brightness):
    ...
    def _position_signature(self, position: List[float]) -> str:
        return self._config_signature(self.problem.search_space.decode(position))

    def _unique_position(self, occupied: set[str], *, max_attempts: int = 64) -> List[float]:
        pos = self.problem.search_space.sample_position(self.rng)
        if not self.config.deduplicate_positions:
            return pos
        sig = self._position_signature(pos)
        attempts = 0
        while sig in occupied and attempts < max_attempts:
            pos = self.problem.search_space.sample_position(self.rng)
            sig = self._position_signature(pos)
            attempts += 1
        return pos
    ...
    def _move_population(self) -> None:
        ...
        occupied: set[str] = set()
        elite_keep = min(self.config.elite_keep, len(self.population))
        for i, ff_i in enumerate(self.population):
            if i < elite_keep:
                elite_pos = list(ff_i.position)
                new_positions.append(elite_pos)
                occupied.add(self._position_signature(elite_pos))
                continue
            ...
            if self.config.deduplicate_positions:
                sig = self._position_signature(x)
                attempts = 0
                while sig in occupied and attempts < 32:
                    x = self.problem.search_space.clip_position(
                        [v + alpha * (self.rng.random() - 0.5) for v in x]
                    )
                    sig = self._position_signature(x)
                    attempts += 1
                if sig in occupied:
                    x = self._unique_position(occupied)
                    sig = self._position_signature(x)
                occupied.add(sig)
    ...
    def _get_pareto_front(self) -> List[FireflyAgent]:
        valid = [ff for ff in self.archive if ff.result is not None and ff.result.status == "ok" and ff.result.constraints_ok]
        if not valid:
            return []
    ...
    def run(self) -> OptimizationResult:
        ...
        try:
            ...
            for it in range(self.config.iterations):
                ...
                self.archive.sort(key=lambda ff: ff.brightness, reverse=True)

                if self.archive and (best is None or self.archive[0].brightness > best.brightness):
                    best = FireflyAgent(
                        idx=self.archive[0].idx,
                        position=list(self.archive[0].position),
                        config=dict(self.archive[0].config or {}),
                        result=self.archive[0].result,
                        brightness=self.archive[0].brightness,
                    )
                ...
                self._write_iteration_row(
                    {
                        ...
                        "alpha": self.config.alpha,
                        "unique_evaluations": len(self.archive),
                        "feasible_unique_evaluations": sum(
                            1
                            for ff in self.archive
                            if ff.result is not None and ff.result.status == "ok" and ff.result.constraints_ok
                        ),
                    }
                )
