...
- Jest pełny zestaw benchmarków CPU (memory/cache/TLB/compute/stream).
...
- Jest rozszerzony pakiet wykresów:
  - wykresy zbiorcze,
  - wykresy szczegolowe per klasa benchmarku,
  - osobne wykresy dla roofline,
  - osobne wykresy dla latencji cache i TLB/page-walk,
  - osobne wykresy dla real kernels.
...
### 5.3 Pointer-chasing i hierarchia cache
Benchmark `run_pointer_latency.py` został rozszerzony tak, aby:
- budować cykl indeksów spójnie w `uint32`,
- zapisywać zarówno `working_set_kb`, jak i `working_set_bytes`,
- wykrywać rozmiary `L1D/L2/L3`,
- przypisywać każdemu working setowi przybliżoną etykietę `estimated_residency`
  (`L1`, `L2`, `L3`, `DRAM`),
- na architekturach heterogenicznych rozróżniać progi rdzeni `Performance`
  i `Efficiency`, np. `P-L1 / E-L2`,
- wspierać wykresy pokazujące przejścia między poziomami pamięci.

Ograniczenie interpretacyjne:
- etykieta poziomu pamięci jest przybliżeniem eksperymentalnym,
- nie należy jej traktować jako bezpośredniego odczytu z licznika sprzętowego.
- na Apple Silicon należy odróżniać `L1 instruction cache` od `L1 data cache`;
  benchmark pointer-chasing dotyczy `L1D`.
...
2. Dostosować `analysis/cpu_summary.py` do finalnego schematu i dodać walidację brakujących kolumn.
3. Dodać warianty stride / TLB-sensitive bardziej szczegółowe niż obecny page-walk benchmark.
...
### 7.1 Co już jest
- [x] Benchmarki CPU: bandwidth (ST/MT), STREAM (ST/MT), pointer latency, TLB/page-walk latency, FMA, FMA peak.
...
- [x] Naprawa typu danych w pointer-chasing (`uint32` spójnie).
- [x] Adnotacja cache hierarchy (`L1/L2/L3/DRAM`) dla benchmarku pointer latency.
- [x] Osobny benchmark TLB / page-walk do rozdzielenia kosztu cache od kosztu translacji stron.
- [x] Pakiet wykresow zbiorczych i szczegolowych dla CPU/GPU/real kernels/roofline.
...
## 9. Pakiet finalnych wykresow publication-grade
...
powstaje zwarty zestaw figur `thesis_core` w:
...
Ta struktura odpowiada nowej zasadzie projektu:
- nie generujemy duzej liczby debugowych lub eksploracyjnych wariantow,
- utrzymujemy maly rdzen figur o wysokiej wartosci naukowej,
- wszystko ma wspierac jedna narracje: mikrobenchmarki -> roofline -> real kernels -> kod Filipa.
