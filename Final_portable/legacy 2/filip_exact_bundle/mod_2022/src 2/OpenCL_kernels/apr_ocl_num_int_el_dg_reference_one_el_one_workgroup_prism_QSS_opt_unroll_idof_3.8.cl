...
// DEFINITIONS DEPENDENT ON OPTIMIZATION OPTIONS
  ...
  //padding
  SCALAR stiff_mat[num_shap_pad*num_shap_pad/WORK_GROUP_SIZE];
  SCALAR load_vec[num_shap_pad/WORK_GROUP_SIZE];
#endif
...
//******************* loop over elements processed by a thread *********************
  for(ielem = 0; ielem < nr_elems_per_thread; ielem++){
    ...
    int element_index = group_id * nr_elems_per_thread + ielem;
    int i;

//    if(group_id==0)
//    {
//    	//printf("proba!\n");
//    	printf((__constant char *) "ielem=%i,element_index=%i\n",ielem, element_index);
//    }
...
//    SCALAR coeff23=workspace[offset+14];
    ...
    if(thread_id<EL_GEO_DAT_SIZE)
    {

		workspace[thread_id] = el_data_in[offset+thread_id];
		// TODO !!!!
		// el_data_in_geo[offset+i*WORK_GROUP_SIZE+thread_id];
    }
    ...
    for(i=0;i<num_geo_dofs;i++){
      ...
      geo_dat[3*i] = workspace[3*i];  //node coor
      geo_dat[3*i+1] = workspace[3*i+1];
      geo_dat[3*i+2] = workspace[3*i+2];
...
//sprawdzic czy czytanie smieci nie przyspieszy tego?
...
//not for dg
    ...
    for(i = 0; i < num_shap_pad*num_shap_pad/WORK_GROUP_SIZE; i++) stiff_mat[i] = zero;

  #ifdef LOAD_VEC_COMP
    for(i = 0; i < num_shap_pad/WORK_GROUP_SIZE; i++) load_vec[i] = zero;
  #endif
...
//      for(idof2 = 0; idof2 < num_shap_pad/WORK_GROUP_SIZE; idof2++){
//full unroll


 //idof=(thread_id*2)+idof2;
...
//}
	...
	workspace[3*thread_id]   = temp1*jac_0+temp2*jac_3+temp3*jac_6;
	workspace[3*thread_id+1] = temp1*jac_1+temp2*jac_4+temp3*jac_7;
	workspace[3*thread_id+2] = temp1*jac_2+temp2*jac_5+temp3*jac_8;
	...
	tab_fun_u_derx[thread_id] = temp1*jac_0+temp2*jac_3+temp3*jac_6;
	tab_fun_u_dery[thread_id] = temp1*jac_1+temp2*jac_4+temp3*jac_7;
	tab_fun_u_derz[thread_id] = temp1*jac_2+temp2*jac_5+temp3*jac_8;

  #endif // end if not USE_WORKSPACE_FOR_SHAPE_FUN

	if(thread_id<62)
	{
		temp1 = shpfun_ref[igauss*4*num_shap+4*WORK_GROUP_SIZE+4*thread_id+1];
		temp2 = shpfun_ref[igauss*4*num_shap+4*WORK_GROUP_SIZE+4*thread_id+2];
		temp3 = shpfun_ref[igauss*4*num_shap+4*WORK_GROUP_SIZE+4*thread_id+3];

		  #ifdef USE_WORKSPACE_FOR_SHAPE_FUN

			workspace[3*WORK_GROUP_SIZE+3*thread_id]   = temp1*jac_0+temp2*jac_3+temp3*jac_6;
			workspace[3*WORK_GROUP_SIZE+3*thread_id+1] = temp1*jac_1+temp2*jac_4+temp3*jac_7;
			workspace[3*WORK_GROUP_SIZE+3*thread_id+2] = temp1*jac_2+temp2*jac_5+temp3*jac_8;
			...
			tab_fun_u_derx[WORK_GROUP_SIZE+thread_id] = temp1*jac_0+temp2*jac_3+temp3*jac_6;
			tab_fun_u_dery[WORK_GROUP_SIZE+thread_id] = temp1*jac_1+temp2*jac_4+temp3*jac_7;
			tab_fun_u_derz[WORK_GROUP_SIZE+thread_id] = temp1*jac_2+temp2*jac_5+temp3*jac_8;

		  #endif // end if not USE_WORKSPACE_FOR_SHAPE_FUN

	}

//	if(thread_id==4 && group_id==0 && igauss==0 && ielem==0)
//	{
//
//		for(idof = 0; idof < num_shap; idof++){
//				printf("workspace[%d]=%lf\n",3*idof,workspace[3*idof]);
//		}
//	}
...
//********************* first loop over shape functions ***********************//
      ...
      //int idof2;
      //barrier(CLK_LOCAL_MEM_FENCE);
 //    for(int idof2 = 0; idof2 < num_shap_pad/WORK_GROUP_SIZE; idof2++){
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF IDOF SHAPE FUNCTION ******//
 ...
 //   	if(idof<num_shap)
//    	{
	  SCALAR shp_fun_u = shpfun_ref[igauss*4*num_shap+4*thread_id];
	  SCALAR fun_u_derx = workspace[3*thread_id];
	  SCALAR fun_u_dery = workspace[3*thread_id+1];
	  SCALAR fun_u_derz = workspace[3*thread_id+2];
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF JDOF SHAPE FUNCTION ******//
	  ...
	  SCALAR fun_v_derx = workspace[3*jdof];
	  SCALAR fun_v_dery = workspace[3*jdof+1];
	  SCALAR fun_v_derz = workspace[3*jdof+2];
...
if(thread_id<62)
{
	...
	//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF IDOF SHAPE FUNCTION ******//
	 ...
	 //   	if(idof<num_shap)
	//    	{
		  SCALAR shp_fun_u = shpfun_ref[igauss*4*num_shap+4*WORK_GROUP_SIZE+4*thread_id];
		  SCALAR fun_u_derx = workspace[3*WORK_GROUP_SIZE+3*thread_id];
		  SCALAR fun_u_dery = workspace[3*WORK_GROUP_SIZE+3*thread_id+1];
		  SCALAR fun_u_derz = workspace[3*WORK_GROUP_SIZE+3*thread_id+2];
...
//-------------------------------------------------------------
    ...
    //******** WRITING OUTPUT SM AND LV TO GLOBAL MEMORY ********//
      ...
      #ifdef USE_WORKSPACE_FOR_STIFF_MAT
    	    stiff_mat_out[offset+thread_id] =
    	      workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+idof*num_dofs+jdof];
      #else

    	    stiff_mat_out[offset+idof2*WORK_GROUP_SIZE+thread_id] = stiff_mat[idof2*num_dofs+jdof];
...
//          }
    	...
    	workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+num_dofs*num_dofs+i];
          ...
          stiff_mat_out[offset+num_shap*num_shap+idof2*WORK_GROUP_SIZE+thread_id] = load_vec[idof2];
...
//        	idof=(thread_id*2)+idof2;
      ...
      #else
    	    //if(idof<num_shap)
    	    	stiff_mat_out[offset+thread_id*num_shap+jdof] = stiff_mat[jdof];
    	    	if(thread_id<62)
    	    		stiff_mat_out[offset+num_shap*WORK_GROUP_SIZE+thread_id*num_shap+jdof] = stiff_mat[num_shap+jdof];
    	    	//barrier(CLK_GLOBAL_MEM_FENCE);
//				if(group_id==0 && ielem==0)
//				{
//				  //printf("ielem=%d, thread_id=%d, idof2=%d, idof=%d, temp4=%lf,fun_v_derx=%lf, vol=%lf \n", ielem, thread_id, idof2, idof, temp4,fun_v_derx,vol);
//					printf("stiff_mat[%d]=%lf\n",idof2*num_shap_org+jdof,stiff_mat[idof2*num_shap_org+jdof]);
//				}
    	...
    	workspace[thread_id*(num_dofs*(num_dofs+1)+PADDING)+num_dofs*num_dofs+i];
          ...
          if(idof<num_shap)
        	  stiff_mat_out[offset+num_shap*num_shap+idof] = load_vec[idof2];
