...
//----------------------------------------------------
  ...
  int nr_elems_per_wg = execution_parameters[0];
...
// if(group_id==0&&thread_id==0&&igauss==0&&ielem==78)
// {
//	 printf("jac_0=%lf\n",jac_0);
//	 printf("jac_1=%lf\n",jac_1);
//	 printf("jac_2=%lf\n",jac_2);
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF IDOF SHAPE FUNCTION ******//
	  ...
	  SCALAR fun_u_derx = workspace[3*idof];
	  SCALAR fun_u_dery = workspace[3*idof+1];
	  SCALAR fun_u_derz = workspace[3*idof+2];

//	  if(group_id==0 && igauss==0 && ielem==0)
//	  	{
//
//	  				printf("workspace[%d]=%lf\n",3*idof,workspace[3*idof]);
//	  	}
...
//	  if(group_id==0 && igauss==0 && ielem==0)
//	  {
//		  printf("ielem=%d, thread_id=%d, idof2=%d, idof=%d, coeff04=%lf,fun_u_derx=%lf, vol=%lf \n", ielem, thread_id, idof2, idof, coeff04,fun_u_derx,vol);
//	  }
...
//          }
      ...
      #else
    	    if(idof<num_shap)
    	    	stiff_mat_out[offset+idof*num_shap+jdof] = stiff_mat[idof2*num_shap+jdof];
    	    	//__syncthreads();
