...
#ifdef USE_WORKSPACE_FOR_STIFF_MAT		//In SSQ it is the same - left only to preserve compability for tests
...
// ******************* READING INPUT DATA *********************
    ...
    for(i=0;i<NR_PDE_COEFFS_SENT;i++) pde_coeff[i]=workspace[offset+i];
    ...
    for(i=0;i<NR_PDE_COEFFS_SENT;i++) pde_coeff[i]=el_data_in[offset+i];
...
//************************* JACOBIAN TERMS CALCULATIONS *************************//
	...
	/* Jacobian matrix J */
      ...
      jac_1 = workspace[offset+3*0];  //node coor
      jac_2 = workspace[offset+3*0+1];
      jac_3 = workspace[offset+3*0+2];
      temp1 = workspace[offset+3*1]   - jac_1;
      temp4 = workspace[offset+3*1+1] - jac_2;
      temp7 = workspace[offset+3*1+2] - jac_3;
      temp2 = workspace[offset+3*2]   - jac_1;
      temp5 = workspace[offset+3*2+1] - jac_2;
      temp8 = workspace[offset+3*2+2] - jac_3;
      temp3 = workspace[offset+3*3]   - jac_1;
      temp6 = workspace[offset+3*3+1] - jac_2;
      temp9 = workspace[offset+3*3+2] - jac_3;
      ...
      jac_2 = geo_dat[3*0+1];
      jac_3 = geo_dat[3*0+2];
      temp1 = geo_dat[3*1]   - jac_1;; 
      temp4 = geo_dat[3*1+1] - jac_2;
      temp7 = geo_dat[3*1+2] - jac_3;
      temp2 = geo_dat[3*2]   - jac_1;; 
      temp5 = geo_dat[3*2+1] - jac_2;
      temp8 = geo_dat[3*2+2] - jac_3;
      temp3 = geo_dat[3*3]   - jac_1;; 
      temp6 = geo_dat[3*3+1] - jac_2;
      temp9 = geo_dat[3*3+2] - jac_3;
...
//************ loop for computing ALL shape function values **********//
	...
	temp1 = shpfun_ref[4*idof+1];
	temp2 = shpfun_ref[4*idof+2];
	temp3 = shpfun_ref[4*idof+3];
...
//************************* LOOP OVER INTEGRATION POINTS ************************//
    ...
    for(igauss = 0; igauss < num_gauss; igauss++){
      ...
      // necessary only for nonlinear problems
      ...
      //SCALAR vol = one_sixth*gauss_dat[0];	// vol = weight CONSTANT FOR TETRA!!!

      // !!!!!!!!!!! vol constant for all integration points - computed earlier !!!!
...
//***** SUBSTITUTING ACTUAL COEFFICIENTS FOR SM AND LV CALCULATIONS *****//
      ...
      SCALAR coeff03 = pde_coeff[igauss];
...
//****** SUBSTITUTING OR COMPUTING GLOBAL DERIVATIVES OF JDOF SHAPE FUNCTION ******//
	...
	// read proper values of shape functions and their derivatives
	SCALAR shp_fun_v = shpfun_ref[igauss*4*num_shap+4*jdof];
	temp1 = shpfun_ref[4*jdof+1];
	temp2 = shpfun_ref[4*jdof+2];
	temp3 = shpfun_ref[4*jdof+3];
...
//-------------------------------------------------------------
 ...
 //-------------------------------------------------------------
			...
			stiff_mat_out[offset+(idof*num_shap+jdof)*WORK_GROUP_SIZE+thread_id] = t;
		#else
			offset = element_index*(num_shap*num_shap+num_shap);
			stiff_mat_out[offset+num_shap*idof+jdof] = t;
		#endif
...
//-------------------------------------------------------------
	...
	#ifdef COAL_WRITE
		offset = (element_index-thread_id)*(num_shap*num_shap+num_shap);
		stiff_mat_out[offset+(num_shap*num_shap+idof)*WORK_GROUP_SIZE+thread_id] = l;
	#else
		offset = element_index*(num_shap*num_shap+num_shap);
		stiff_mat_out[offset+num_shap*num_shap+idof] = l;
	#endif
