...
def _run_backend(backend: str, device_index: int, sizes: List[int], runs: int, warmups: int, dtype: str) -> None:
    if backend == "cpu":
        be = CpuRealBackend()
    ...
    csv_path = make_csv_path("reduction", backend, device_name, device_index)
    rows: List[Dict[str, object]] = []

    print(f"=== REAL REDUCTION ({backend}) ===")
    print(f"device: {device_name} (index {device_index})")
    print(f"runs  : {runs}, warmups: {warmups}")

    for n in sizes:
        print(f"\n--- n: {n} ---")
        run_warmups(warmups, lambda: be.reduction(n=n, dtype=dtype))
        for run_idx in range(runs):
            ...
            gbps = float("nan")
            try:
                elapsed, gbps = be.reduction(n=n, dtype=dtype)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Real-kernel reduction benchmark (separate module).")
