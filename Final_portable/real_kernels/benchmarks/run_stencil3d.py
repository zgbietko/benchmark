...
def _parse_shapes(raw: str) -> List[Tuple[int, int, int]]:
    ...
    for part in raw.split(","):
        ...
        if not p:
            continue
        d, h, w = [int(x) for x in p.split("x")]
        out.append((d, h, w))
    return out


def _run_backend(backend: str, device_index: int, shapes: List[Tuple[int, int, int]], runs: int, warmups: int, iters: int, dtype: str) -> None:
    if backend == "cpu":
        be = CpuRealBackend()
    ...
    csv_path = make_csv_path("stencil3d", backend, device_name, device_index)
    rows: List[Dict[str, object]] = []
    print(f"=== REAL STENCIL3D ({backend}) ===")
    print(f"device: {device_name} (index {device_index})")
    print(f"runs  : {runs}, warmups: {warmups}, iters: {iters}")

    for d, h, w in shapes:
        print(f"\n--- shape: {d}x{h}x{w} ---")
        run_warmups(warmups, lambda: be.stencil3d(d=d, h=h, w=w, iters=iters, dtype=dtype))
        for run_idx in range(runs):
            ...
            gbps = float("nan")
            try:
                elapsed, gbps = be.stencil3d(d=d, h=h, w=w, iters=iters, dtype=dtype)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Real-kernel stencil3d benchmark.")
    ...
    ap.add_argument("--shapes", type=str, default="64x128x128,128x128x128")
    args = ap.parse_args()
    ...
    for b in backends:
        try:
            _run_backend(b, args.device_index, shapes, args.runs, args.warmups, args.iters, args.dtype)
        except Exception as e:
