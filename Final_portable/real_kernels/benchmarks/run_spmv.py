...
def _run_backend(backend: str, device_index: int, ns: List[int], nnz_per_row: int, runs: int, warmups: int, dtype: str) -> None:
    if backend == "cpu":
        be = CpuRealBackend()
    ...
    print(f"runs  : {runs}, warmups: {warmups}, nnz/row: {nnz_per_row}")

    for n in ns:
        print(f"\n--- n: {n} ---")
        run_warmups(warmups, lambda: be.spmv(n=n, nnz_per_row=nnz_per_row, dtype=dtype))
