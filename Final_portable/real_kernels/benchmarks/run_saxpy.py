...
def _run_backend(backend: str, device_index: int, sizes: List[int], runs: int, warmups: int, alpha: float, dtype: str) -> None:
    if backend == "cpu":
        be = CpuRealBackend()
    ...
    csv_path = make_csv_path("saxpy", backend, device_name, device_index)
    rows: List[Dict[str, object]] = []

    print(f"=== REAL SAXPY ({backend}) ===")
    print(f"device: {device_name} (index {device_index})")
    print(f"runs  : {runs}, warmups: {warmups}, alpha: {alpha}")

    for n in sizes:
        print(f"\n--- n: {n} ---")
        run_warmups(warmups, lambda: be.saxpy(n=n, a=alpha, dtype=dtype))
        for run_idx in range(runs):
            ...
            gbps = float("nan")
            try:
                elapsed, gbps = be.saxpy(n=n, a=alpha, dtype=dtype)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Real-kernel SAXPY benchmark.")
    ...
    ap.add_argument("--sizes", type=str, default="1000000,5000000,10000000,50000000")
