...
/*kew*/
  ...
  int start_dof = 0;
  for (ibl = 0; ibl< nr_dof_ent; ibl++){
	  level_p->l_dof_struct[ibl].block_id = ibl;
	  level_p->l_dof_struct[ibl].posglob = start_dof;
	  start_dof += level_p->l_dof_struct[ibl].nrdofs;
...
/*kew*/
	...
	//printf("Przed assemblingiem w solverze: Solver_id=%d, level_id=%d, SIC_SOLVE=%d, nr_dof_ent=%d,rewrite=%c\n",Solver_id, level_id, SIC_SOLVE,
	//			   nr_dof_ent, rewrite);
