/************************************************************************
File pds_ns_supg_heat_vof_adapt.c - adaptation functions for ns_supg_heat_vof module

procedures in this module use the procedures from component ns_supg, heat and vof
...
Contains declarations of routines:
  ...
  pdr_ns_supg_heat_vof_err_indi_explicit
...
History:
	...
	03.2013 - Aleksander Siwek (Aleksander.Siwek@agh.edu.pl)
*************************************************************************/
...
int pdv_ns_supg_heat_vof_switch_adapt=0;
...
pdr_ns_supg_heat_vof_adapt - to enforce adaptation strategy for a given problem 
---------------------------------------------------------*/
int pdr_ns_supg_heat_vof_adapt(  /* returns: >0 - success, <=0 - failure */
  char* Work_dir,
...
/*++++++++++++++++ executable statements ++++++++++++++++*/
  ...
  printf("\nStarting adaptation in pdr_ns_supg_heat_vof_adapt.\n");
...
/* get adaptation parameters */
  ...
  if (type == PDC_ADAPT_EXACT) {
    printf("No exact solution for ns_supg_heat_vof problem!.\n");
...
/*
    pdv_ns_supg_heat_vof_switch_adapt = 1 - pdv_ns_supg_heat_vof_switch_adapt;
    if ( pdv_ns_supg_heat_vof_switch_adapt ){
      printf("\nEven time step: HEAT as master problem.\n");
...
*/
    pdv_ns_supg_heat_vof_switch_adapt = pdv_ns_supg_heat_vof_switch_adapt % 3;
    switch( pdv_ns_supg_heat_vof_switch_adapt ) {
	case 0:
	    printf("\nHEAT as master problem.\n");
	    pdv_ns_supg_heat_vof_current_problem_id = PDC_HEAT_ID;
	    problem_id = PDC_HEAT_ID;
	    break;
	case 1:
	    printf("\nNS_SUPG as master problem.\n");
	    pdv_ns_supg_heat_vof_current_problem_id = PDC_NS_SUPG_ID;
	    problem_id = PDC_NS_SUPG_ID;
	    break;
	case 2:
	    printf("\nVOF as master problem.\n");
    ...
    pdv_ns_supg_heat_vof_switch_adapt++;
    ...
    //utr_recover_derivatives(field_id, nr_patches, pdv_patches);
    ...
    pdr_ns_supg_heat_vof_refine(problem_id, type, Interactive_output); 
...
/* local variables */
  ...
  double clock_time;
  int problem_id, name, field_id, sol_vec_id;
...
/*kew*/
  ...
  while ((nel = mmr_get_next_act_elem(mesh_id, nel)) != 0) {
    el_err = pdr_ns_supg_heat_vof_err_indi_ZZ(field_id, nel);
...
pdr_ns_supg_heat_vof_err_indi_ZZ - to return error indicator for an element
----------------------------------------------------------*/
double pdr_ns_supg_heat_vof_err_indi_ZZ(/* returns error indicator for an element */
  int Problem_id, 
...
pdr_ns_supg_heat_vof_err_indi_explicit - to return error indicator for an element
  // NOT IMPLEMENTED 
----------------------------------------------------------*/
double pdr_ns_supg_heat_vof_err_indi_explicit(	
...
/*---------------------------------------------------------
pdr_ns_supg_heat_vof_refine - to enforce mesh refinement for ns_supg+heat problem 
---------------------------------------------------------*/
int pdr_ns_supg_heat_vof_refine(  /* returns: >0 - success, <=0 - failure */
...
/* get necessary mesh parameters */
  ...
  if (Ref_type == -1) {
    ...
    if(Problem_id==PDC_NS_SUPG_ID) {
      int field_heat = pdr_ctrl_i_params(PDC_HEAT_ID, 3);
      apr_proj_dof_ref(field_heat, iaux, nmel, nmfa, nmed, nmno);
      ...
      apr_proj_dof_ref(field_vof, iaux, nmel, nmfa, nmed, nmno);

      int field_mat = pdr_ctrl_i_params(PDC_MAT_ID, 3);
      apr_proj_dof_ref(field_mat, iaux, nmel, nmfa, nmed, nmno);
      // TODO: what if other fields also needs to be extended?
    } else if(Problem_id==PDC_HEAT_ID) {
      int field_ns_supg = pdr_ctrl_i_params(PDC_NS_SUPG_ID, 3);
      apr_proj_dof_ref(field_ns_supg, iaux, nmel, nmfa, nmed, nmno);
...
*/    // TODO: what if other fields also needs to be extended?
  ...
  /* refinement in a given area */
  else if (Ref_type == -2) {

	int nr_err=0, nr_no=0;
...
/* compute error indicators for all active elements */
    ...
    while((iel=mmr_get_next_act_elem(mesh_id, iel))!=0){
	  ...
	  if ( fabs(node_coor[2]-0.5) <= heigh ) {
		error_indi[iel] = PDC_VOF_ERROR_INDI;
		++nr_err;
	  }
	...
	nr_no = nr_elem-nr_err;
	heigh /= 2.3;
/*kbw*/
	printf("\nIn utr_adapt( Ref_type=-2 ): total error %lf (sqrt %lf), average error %lf\n",
	   sum_error, sqrt(sum_error), average_error);
	printf("\tnr_elem = %d\tnr_err = %d\tnr_no = %d\n", nr_elem, nr_err, nr_no);
/*kew*/
...
/*kew*/
 ...
 /* 	   sol_dofs[num_dofs]>query_result.temp_solidus){	     */
  ...
  /* end of refinement in a given area */
...
/*kew*/
    ...
    average_error = sum_error/nr_elem;
/*kbw*/
	printf("In utr_adapt (pdr_err_indi): total error %lf (sqrt %lf), average error %lf\n",
	   sum_error, sqrt(sum_error), average_error);
/*kew*/
