...
//             one work_group = one or more elements processed in a sequence
  ...
  for(ielem = 0; ielem < nr_elems_per_work_group; ielem++){
	...
	for(igauss = 0; igauss < num_gauss; igauss++){
	  ...
	  // variant 1: global derivatives computed in a separate loop - PARALLELIZED
	  LOCAL shape_fun_real_der[3*num_shap];
	  if(thread_id < num_shap){
	    
	    // read proper values of shape functions and their derivatives
	    SCALAR temp1 = shape_fun_workspace[igauss*4*num_shap+4*thread_id+1];
	    SCALAR temp2 = shape_fun_workspace[igauss*4*num_shap+4*thread_id+2];
	    SCALAR temp3 = shape_fun_workspace[igauss*4*num_shap+4*thread_id+3];
	    shape_fun_real_der[3*thread_id]   = temp1*jac_0 + temp2*jac_3 + temp3*jac_6;
	    shape_fun_real_der[3*thread_id+1] = temp1*jac_1 + temp2*jac_4 + temp3*jac_7;
	    shape_fun_real_der[3*thread_id+2] = temp1*jac_2 + temp2*jac_5 + temp3*jac_8;
...
// 7 or 10 operations - laplace
	...
	if(thread_id<num_shap){
	  ...
	  //stiff_mat_out[offset+num_shap*num_shap+thread_id] = pde_coeff_workspace[3];
	}
