...
//#define FLOAT
  ...
  #define two 2.0f
  #define half 0.5f
  #define one_sixth 0.16666666666666666666666666666667f;
#else
...
//******************* loop over elements processed by a thread *********************
  for(ielem = 0; ielem < nr_elems_per_thread; ielem++){
    ...
    { // the scope of the registers below
      SCALAR temp1 = zero;
      SCALAR temp2 = zero;
      SCALAR temp3 = zero;
      SCALAR temp8 = zero;
      ...
      jac_1 = workspace[offset];  //node coor
      jac_2 = workspace[offset+1];
      jac_3 = workspace[offset+2];
      ...
      jac_1 = el_data_in[offset];  //node coor
      jac_2 = el_data_in[offset+1];
      jac_3 = el_data_in[offset+2];
...
//      SCALAR eaux = gauss_dat[4*igauss+2];
      SCALAR vol = gauss_dat[igauss];	// vol = weight

      /* Jacobian calculations - |J| and inverse of the Jacobian matrix*/
      vol *= daux*one_sixth; // vol = weight * det J
...
#ifndef USE_SHAPE_FUN_REF_DIRECTLY
 ...
 //************ loop for computing ALL shape function values at integration point **********//
	...
	#ifdef COUNT_OPER
	 nr_access_shared += (2)*num_shap; // 1 reads from constant cache and 1 writes to shared memory
	#endif

#endif //if not USE_SHAPE_FUN_REF_DIRECTLY
	...
	temp7 = coeff30*shape_fun_workspace[thread_id*(4*num_shap+PADDING)+4*idof+1] +
        ...
        #endif //USE_SHAPE_FUN_REF_DIRECTLY
