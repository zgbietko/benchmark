...
Contains routines:   
  mmr_divide_el4_p - to break a prismatic element into 4 sons
  mmr_clust_el4_p - to cluster back a family of eight prisms
...
/*------------------------------------------------------------
  mmr_divide_el4_p - to break a prismatic element into 4 sons
------------------------------------------------------------*/
int mmr_divide_el4_p( /* returns: >=0 - success code, <0 - error code */
...
/* put face edges on a list of future faces' edges */
        ...
        iaux=mmr_divide_face2_q(Mesh_id, abs(face), face_sons, 
				sons_edges, sons_new_nodes);
				//, NULL);

/*kbw
	printf("Divided face %d, bc (or shift) %d \n", 
	       face, mesh->face[abs(face)].bc);
	printf("Face_sons: %d, %d\nSons edges: ", face_sons[0], face_sons[1]);
	for(i=0;i<8;i++){
	  printf("%d, ",sons_edges[i]);
	}
	printf("\nSons_new_nodes: ");
	for(i=0;i<2;i++){
	  printf("%d, ",sons_new_nodes[i]);
	}
	printf("\n");
/*kew*/
...
/* put face sons on a list of future elements' faces */
          out_vface[2*(ifa-2)]    =  face_sons[0];
          out_vface[2*(ifa-2)+1]  =  face_sons[1];

          vert_edges[(ifa-2)]   =  sons_edges[1];
...
/* put face sons on a list of future elements' faces */
            ...
            out_vface[2*(ifa-2)]    = -face_sons[1];
            out_vface[2*(ifa-2)+1]  = -face_sons[0];
...
#ifdef DEBUG_MMM
            printf("321Face_shift!=1 for vertical, quadrilateral face %d !\n",-face);
...
/*kew*/
	...
	exit(-1);
      ...
      if(ifa<2) num_sons_face = 4;
      else num_sons_face = 2;
...
/* if the second neighbor is the boundary */
          for(i=0;i<num_sons_face;i++) 
	    mesh->face[face_sons[i]].neig[1]=MMC_BOUNDARY;
        }
        else if(mesh->face[abs(face)].neig[1]==MMC_SUB_BND){
/* if the second neighbor is the intersubdomain boundary */
          for(i=0;i<num_sons_face;i++) 
	    mesh->face[face_sons[i]].neig[1] = MMC_SUB_BND;
        }
        else{
          for(i=0;i<num_sons_face;i++) 
	    mesh->face[face_sons[i]].neig[1] = MMC_BIG_NGB;
        }
...
/* if the second neighbor is the intersubdomain boundary */
          for(i=0;i<num_sons_face;i++) 
	    mesh->face[face_sons[i]].neig[0] = MMC_SUB_BND;
        }
        else{
          for(i=0;i<num_sons_face;i++) 
	    mesh->face[face_sons[i]].neig[0] = MMC_BIG_NGB;
        }
...
/* put face sons on a list of future elements' faces */
	  ...
	  out_vface[2*(ifa-2)]=face_sons[1];
	  out_vface[2*(ifa-2)+1]=face_sons[2];
...
/* put face sons on a list of future elements' faces */
	    ...
	    out_vface[2*(ifa-2)]=-face_sons[2];
	    out_vface[2*(ifa-2)+1]=-face_sons[1];
	    
          }
          else {
#ifdef DEBUG_MMM
            printf("123Face_shift!=1 for vertical, quadrilateral face %d !\n",-face);
            printf("Use half structured meshes with shift==1 or modify\n");
            printf("mesh modification routines.\n");
#endif
          }
        ...
        if(face>0){
          vert_edges[(ifa-2)]   =  orient[1]*sons_edges[1];
        }
        else if(face<0){
          shift = -mesh->face[-face].bc;
	  vert_edges[(ifa-2)]   =  orient[1]*sons_edges[1];
      ...
      printf("break4_p - error in element faces\n");
      exit(-1);
    }
...
/* vertical faces */
    ...
    edges[1] =  vert_edges[ifa];
    edges[2] = -base_edges[ifa+3];
    edges[3] = -vert_edges[(ifa+2)%3];
    vert_faces[ifa]=mmr_create_face(Mesh_id, MMC_QUAD, iaux, edges, 
				      NULL, NULL);
				      //, new_ipid);
  }
...
/*kbw
  //if(El==8){
      ...
      printf("\n");
      printf("vertical faces:");
      for(i=0;i<6;i++) printf("  %d", vert_faces[i]);
      printf("\n");
...
/* create data structures for sons */
  mesh->elem[El].refi = MMC_REF_ANI; /* indicate isotropic refinement */
  mesh->elem[El].sons = mmr_ivector(num_sons,"sons in divide_el4_p");
...
/* small check - whether we have space for new element */
      if(mesh->elem[pel].type!=MMC_FREE){
        printf("BREAK4: old pointer %d points not into free space\n",
			pel);
        exit(-1);
      }
#endif
...
/* upper base */
      mesh->elem[pel].face[1] = out_hface[4+ison];
/* sides */
      mesh->elem[pel].face[2+ison] = out_vface[2*ison];
      mesh->elem[pel].face[2+(ison+1)%3] = -vert_faces[ison];
      mesh->elem[pel].face[2+(ison+2)%3] = out_vface[2*((ison+2)%3)+1];
    }
    else if(ison==3){
/* lower base */
      mesh->elem[pel].face[0] = out_hface[ison];
...
/* sides */
      mesh->elem[pel].face[2] = vert_faces[2];
      mesh->elem[pel].face[3] = vert_faces[0];
      mesh->elem[pel].face[4] = vert_faces[1];
    }
...
/*------------------------------------------------------------
mmr_clust_el4_p - to cluster back a family of four prisms
------------------------------------------------------------*/  
int mmr_clust_el4_p( /* returns: >=0 - success code, <0 - error code */
  int Mesh_id,    /* in: mesh ID or 0 (MMC_CUR_MESH_ID) for the current mesh */
  int El          /* in: element ID */
  )
{
...
/* first check whether element is active */
  if(mmr_el_status(Mesh_id,El)!=MMC_ACTIVE){
    ...
    if(iprint>MMC_PRINT_ERRORS){
      printf("not active element or free space %d in clust_el4_p!\n", El);
    }
#endif
    return(-2);
  }
...
/* small check */
  if(fath==MMC_NO_FATH){
#ifdef DEBUG_MMM
    printf("Attempt to clust initial mesh element; in clust_el4_p\n");
#endif
    return(-3);
  }
...
/* check whether all sons are active and create list of sons */
  for(ison=0;ison<num_sons;ison++){
    ...
    if(mmr_el_status(Mesh_id,son)==MMC_INACTIVE){
      printf("not active brother %d of %d in clust_el4_p!\n",son,El);
      return(-1);
    }
    if(mesh->elem[son].sons!=NULL){
      printf("son %d of %d with sons in clust_el4_p!\n",son,El);
      return(-1);
    }
...
/*kew*/
	  ...
	  if(mmr_fa_sub_bnd(Mesh_id,face)){
	    ...
	    else mmr_clust_fa2_q(Mesh_id, face, NULL);
...
#ifdef DEBUG_MMM
	printf("clust_el_4p: max_gen_diff barrier not implemented for specified case!\n"); 
#endif
...
/* checking */
#ifdef DEBUG_MMM
  if(mesh->elem[fath].refi!=MMC_REF_ANI){
    printf("attempt to cluster back anisotropically element %d refined isotropically\n",fath); 
    exit(-1);
  }
#endif
...
/*kbw
      if(face!=0){
	printf("clustering back elem %d, face %d on inters. bound.\n", 
	       fath, face);
      }
/*kew*/
...
/*kew*/
	...
	printf("error in neighbors for element %d in clust_el4_p!\n", El);
	exit(-1);
      }
