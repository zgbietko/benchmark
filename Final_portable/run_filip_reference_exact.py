...
from analysis.filip_output_decode import write_decoded_output
from analysis.provenance import collect_runtime_provenance, file_manifest, sha256_bytes, sha256_json
...
from scripts.export_canonical_replay_bundles import export_canonical_replay_bundles
from scripts.export_filip_replay_inputs import (
    discover_case_dirs,
    export_replay_input_bundle,
    resolve_dump_root as resolve_input_dump_root,
)
...
def _candidate_replay_roots() -> list[Path]:
    candidates: list[Path] = []
    explicit = str(os.environ.get("FILIP_REPLAY_DUMP_ROOT", "")).strip()
    if explicit:
        candidates.append(Path(explicit).expanduser())
    for search_root in (ROOT / "data", ROOT.parent / "v4" / "data"):
        if not search_root.exists():
            continue
        for manifest in search_root.rglob("replay_bundle_manifest.json"):
            candidates.append(manifest.parent)
        for launch_root in search_root.rglob("launch_dumps"):
            candidates.append(launch_root)
    dedup: list[Path] = []
    seen: set[str] = set()
    ...
    dedup.sort(key=lambda path: path.stat().st_mtime if path.exists() else 0.0, reverse=True)
    return dedup


def _replay_root_supports_request(*, candidate: Path, case_specs: list[ExactCaseSpec], variants: list[str]) -> bool:
    try:
        dump_root = resolve_input_dump_root(candidate)
        case_dirs = discover_case_dirs(dump_root)
    except Exception:
        return False
    requested_variants = [str(v).strip().lower() for v in variants if str(v).strip()]
    for case in case_specs:
        case_dir = case_dirs.get(case.case_name)
        ...
        for variant in requested_variants:
            ...
            if not variant_dir.is_dir():
                return False
            try:
                has_options = any(child.is_dir() and child.name.startswith("opt_") for child in variant_dir.iterdir())
            except OSError:
                return False
            if not has_options:
                return False
...
def _auto_detect_replay_dump_root(*, case_specs: list[ExactCaseSpec], variants: list[str]) -> Path | None:
    for candidate in _candidate_replay_roots():
        if _replay_root_supports_request(candidate=candidate, case_specs=case_specs, variants=variants):
            return candidate
    return None
...
def _experiment_identity(
    *,
    ...
    return {
        ...
        "numerical_equivalence": str(numerical_equivalence),
...
def _maybe_export_canonical_replay_bundles(
    *,
    enabled: bool,
    include_expected_output: bool,
    source_root: Path | None,
    ...
    if not bool(enabled):
        return {}
    if source_root is None:
        raise SystemExit("Canonical replay bundle export requires OpenCL launch dumps, but no launch_dumps root was produced.")
    source_root = _require_path(source_root, "OpenCL launch_dumps root for canonical replay bundles")
    bundle_root = out_dir / "canonical_replay_bundles"
    ...
    return {
        "available": True,
        "bundle_root": str(manifest.get("bundle_root", bundle_root)),
        "manifest_path": str(manifest.get("manifest_path", bundle_root / "canonical_bundles_manifest.json")),
        "include_expected_output": bool(manifest.get("include_expected_output", include_expected_output)),
...
def sha256_file_or_bytes(*, path: Path | None = None, arr: np.ndarray | None = None) -> str:
    if path is not None and Path(path).exists():
        return file_manifest(Path(path)).get("sha256", "")
    if arr is not None:
        flat = np.ascontiguousarray(arr, dtype=np.float32).reshape(-1)
        return sha256_bytes(flat.tobytes())
    return ""


def _load_launch_metadata(*, output_dir: Path, source_dump_dir: Path | None = None, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
    if isinstance(metadata, dict) and metadata:
        return dict(metadata)
    candidates: list[Path] = []
    if source_dump_dir is not None:
        candidates.append(Path(source_dump_dir) / "launch_meta.json")
    candidates.append(Path(output_dir) / "launch_meta.json")
    for candidate in candidates:
        if candidate.exists():
            try:
...
def _write_output_preview(
    *,
    ...
    preview = {
        ...
        "output_sha256": sha256_file_or_bytes(path=output_path, arr=arr),
        **_array_preview(arr),
    }
    launch_metadata = _load_launch_metadata(output_dir=output_dir, source_dump_dir=source_dump_dir, metadata=metadata)
    if launch_metadata:
        preview["launch_metadata"] = dict(launch_metadata)
    ...
    decoded = write_decoded_output(output_dir, arr, launch_metadata)
    if decoded:
        preview["decoded_output"] = dict(decoded)
    ...
    return {
        ...
        "output_sha256": str(preview.get("output_sha256", "")),
        "decoded_output": dict(decoded) if decoded else {},
    }
...
def _exact_numerical_output_summary(
    *,
    ...
    example_decoded = output_rows[0]["numerical_output"].get("decoded_output", {})
    if isinstance(example_decoded, dict) and example_decoded.get("available"):
        payload["example_decoded_summary"] = str(example_decoded.get("summary_path", ""))
        payload["example_decoded_matrix_csv"] = str(example_decoded.get("matrix_csv", ""))
        payload["example_decoded_rhs_csv"] = str(example_decoded.get("rhs_csv", ""))
    ...
    for record in output_rows:
        if (
            ...
            decoded = record["numerical_output"].get("decoded_output", {})
            if isinstance(decoded, dict) and decoded.get("available"):
                payload["best_decoded_summary"] = str(decoded.get("summary_path", ""))
                payload["best_decoded_matrix_csv"] = str(decoded.get("matrix_csv", ""))
                payload["best_decoded_rhs_csv"] = str(decoded.get("rhs_csv", ""))
            break
    return payload
    ...
    *,
    ...
    meta.update(
        _experiment_identity(
            experiment_class="correctness_replay",
            mode_label="Metal replay from frozen OpenCL inputs",
            numerical_equivalence="same_frozen_inputs_with_expected_output_validation",
    ...
    *,
            ...
            for option_index, option_row in enumerate(option_rows):
                ...
                if output_buffer is not None:
                    record["numerical_output"] = _save_output_artifacts_from_array(
                        ...
                        metadata={
                            "n_elements_hint": int(cfg_eff.get("n_elements", 0)),
                            "backend_mode": "native_performance_campaign",
                        },
                    )
    ...
    meta.update(
        _experiment_identity(
            ...
            mode_label="Metal native campaign",
            numerical_equivalence="not_numerically_equivalent_to_opencl_exact",
            comparison_note=meta["comparison_note"],
        )
    )
    return records, meta
...
def main() -> None:
    ...
    ap.add_argument("--export-canonical-replay-bundles", action="store_true", help="OpenCL exact only: export a small set of canonical replay bundles for regression and dissertation figures.")
    ...
    if resolved_backend == "metal":
        ...
        replay_dump_root_source = "user_provided" if replay_dump_root is not None else "none"
        if replay_dump_root is None:
            auto_replay_root = _auto_detect_replay_dump_root(case_specs=normalized_case_specs, variants=variants)
            if auto_replay_root is not None:
                replay_dump_root = auto_replay_root
                replay_dump_root_source = "auto_detected"
                print(f"auto replay dump root : {replay_dump_root}")
        ...
        summary = {
            ...
            "replay_dump_root_source": replay_dump_root_source,
        ...
        summary.update(
            {
                "experiment_class": str(metal_meta.get("experiment_class", "")),
                "mode_label": str(metal_meta.get("mode_label", "")),
                "numerical_equivalence": str(metal_meta.get("numerical_equivalence", "")),
        ...
        canonical_replay_bundles = _maybe_export_canonical_replay_bundles(
            enabled=bool(args.export_canonical_replay_bundles),
            include_expected_output=bool(args.export_replay_include_expected_output),
            source_root=dump_launch_root,
            out_dir=out_dir,
        )
        ...
        summary = {
            ...
            "canonical_replay_bundles": canonical_replay_bundles,
            "numerical_outputs": _exact_numerical_output_summary(
                records=records,
                root=numerical_outputs_root,
                best_overall=best_overall,
        ...
        summary.update(
            _experiment_identity(
                experiment_class="reference_exact",
                mode_label="Legacy OpenCL exact reference",
                numerical_equivalence="source_of_truth",
                comparison_note=str(summary["comparison_note"]),
            )
        )
        summary["provenance"] = collect_runtime_provenance(
            ...
            extra_files={
                "combined_csv": combined_csv,
                **{f"built_binary_{arch}": path for arch, path in built_arches.items()},
            },
        )
        summary["summary_hash"] = sha256_json({k: v for k, v in summary.items() if k != "summary_hash"})
    ...
    try:
        ...
        summary["figure_set_dir"] = plot_summary.get("thesis_core_dir", plot_summary.get("plots_dir", ""))
        summary["figure_set_plots"] = plot_summary.get("figure_paths", plot_summary.get("generated_plots", []))
        summary["figure_appendix_dir"] = plot_summary.get("appendix_dir", "")
        summary["figure_appendix_plots"] = plot_summary.get("appendix_figure_paths", plot_summary.get("appendix_plots", []))
        summary["figure_manifest_path"] = plot_summary.get("manifest_path", "")
    except Exception as exc:
        summary["article_plots_error"] = str(exc)
    summary["summary_hash"] = sha256_json({k: v for k, v in summary.items() if k != "summary_hash"})
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=True), encoding="utf-8")
