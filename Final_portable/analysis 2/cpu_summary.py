...
try:
    from analysis.cache_latency import latency_summary_by_residency
except Exception:
    from cache_latency import latency_summary_by_residency
...
def _infer_benchmark_from_filename(name: str) -> str:
    ...
    if name.startswith("tlb_latency"):
        return "tlb_latency"
...
def _print_scaling_digest(best_by_threads: dict[int, float], unit: str) -> None:
    if not best_by_threads:
        return
    thread_counts = sorted(best_by_threads.keys())
    single = best_by_threads.get(1)
    max_threads = max(thread_counts)
    all_core = best_by_threads.get(max_threads)
    if single is not None:
        print(f"             1T peak:      {single:7.2f} {unit}")
    if all_core is not None:
        print(f"             all-core({max_threads:2d}): {all_core:7.2f} {unit}")
        print(f"             per-thread:   {all_core / max_threads:7.2f} {unit}/thread")
    if single is not None and all_core is not None and max_threads > 1 and single > 0:
        speedup = all_core / single
        eff = speedup / max_threads
        print(f"             speedup:      {speedup:7.2f}x, efficiency: {eff:5.2%}")
...
def _summarize_bandwidth(rows: list[dict[str, Any]], title: str, key_extra: list[str] | None = None) -> None:
    ...
    if key_extra:
        digest: dict[tuple[str, str], dict[int, float]] = defaultdict(dict)
        for key, items in groups.items():
            arch, cpu, _size_mb, *rest = key
            if not rest:
                continue
            th = _to_int(rest[0])
            if th is None:
                continue
            vals = [_to_float(r.get("gbps")) for r in items]
            cleaned = [v for v in vals if v is not None]
            if not cleaned:
                continue
            digest[(arch, cpu)][th] = max(digest[(arch, cpu)].get(th, 0.0), max(cleaned))
        for (arch, cpu), best_by_threads in sorted(digest.items()):
            print(f"             scaling digest | {arch} | {cpu}")
            _print_scaling_digest(best_by_threads, "GB/s")
...
def _summarize_pointer_latency(rows: list[dict[str, Any]]) -> None:
    ...
    residency_summary = latency_summary_by_residency(rows)
    if residency_summary:
        print("\n    Przybliżone strefy hierarchii pamięci:")
        for item in residency_summary:
            print(
                "    "
                f"{item['residency']:>5s} | "
                f"{item['working_set_kb_min']:8.1f}-{item['working_set_kb_max']:8.1f} KB | "
                f"{item['latency_ns_mean']:7.2f} ns | "
                f"points={item['n_points']}"
            )


def _summarize_tlb_latency(rows: list[dict[str, Any]]) -> None:
    print("\n=== Latencja TLB / page-walk (tlb_latency_*.csv) ===")
    rows = _latest_session_filter(rows, key_fields=["arch", "cpu_model"])
    ...
    for r in rows:
        arch = r.get("arch", "?")
        cpu = r.get("cpu_model", "?")
        pages = _to_int(r.get("pages_touched"))
        page_size = _to_int(r.get("page_size_bytes"))
        if pages is None or page_size is None:
            continue
        groups[(arch, cpu, pages, page_size)].append(r)

    for (arch, cpu, pages, page_size) in sorted(groups.keys(), key=lambda x: (x[0], x[1], int(x[2]), int(x[3]))):
        vals = []
        e_vals = []
        p_vals = []
        for r in groups[(arch, cpu, pages, page_size)]:
            v = _to_float(r.get("latency_ns"))
            if v is not None:
                vals.append(v)
            e = _to_float(r.get("energy_j"))
        ...
        print(
            f"{arch:8s} | {cpu:30s} | "
            f"pages={pages:6d} | page={page_size:5d} B | {m:7.2f} ns ± {s:6.2f}"
        )
        _print_energy_line(e_vals, p_vals)
...
def _summarize_compute_fma_peak(rows: list[dict[str, Any]]) -> None:
    ...
    per_cpu_threads: dict[tuple[str, str], dict[int, float]] = defaultdict(dict)
    for (arch, cpu, th) in sorted(best.keys(), key=lambda x: (x[0], x[1], int(x[2]))):
        ...
        e = _to_float(r.get("energy_j"))
        p = _to_float(r.get("avg_power_w"))
        per_cpu_threads[(arch, cpu)][int(th)] = g
    ...
    for (arch, cpu), best_by_threads in sorted(per_cpu_threads.items()):
        print(f"             scaling digest | {arch} | {cpu}")
        _print_scaling_digest(best_by_threads, "GF/s")
...
def _summarize_stream(rows: list[dict[str, Any]], title: str, mt: bool) -> None:
    ...
    if mt:
        digest: dict[tuple[str, str, str], dict[int, float]] = defaultdict(dict)
        for (arch, cpu, kernel, _size_mb, th), items in groups.items():
            vals = [_to_float(r.get("gbps")) for r in items]
            cleaned = [v for v in vals if v is not None]
            if not cleaned:
                continue
            digest[(arch, cpu, kernel)][int(th)] = max(digest[(arch, cpu, kernel)].get(int(th), 0.0), max(cleaned))
        for (arch, cpu, kernel), best_by_threads in sorted(digest.items()):
            print(f"             scaling digest | {arch} | {cpu} | {kernel}")
            _print_scaling_digest(best_by_threads, "GB/s")
...
def main() -> None:
    ...
    if "tlb_latency" in by_bench:
        _summarize_tlb_latency(by_bench["tlb_latency"])
