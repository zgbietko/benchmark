...
def main() -> None:
        ...
        with p.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in r:
                ...
                if kernel == "gemm":
                    ...
                    if g is not None:
                        gemm[key].append(g)
                elif kernel == "assembly_like":
                    key = (
                        backend,
                        device,
                        int(float(row.get("n_elements", 0))),
                        int(float(row.get("n_qp", 0))),
                        int(float(row.get("n_dofs", 0))),
                        str(row.get("variant", "qss")),
                        int(float(row.get("use_workspace", 0))),
                        int(float(row.get("scatter_accumulate", 0))),
                        int(float(row.get("padding", 0))),
                    )
                    g = _to_float(row.get("gflops"))
                    b = _to_float(row.get("throughput_gbps"))
                    ai = _to_float(row.get("ai_flop_per_byte"))
                    if g is not None and b is not None:
                        assembly[key].append((g, b, ai if ai is not None else float("nan")))
    ...
    if assembly:
        print("\n[Author Assembly-Like] mean ± sigma")
        for k in sorted(assembly.keys()):
            vals = assembly[k]
            ...
            backend, device, n_elem, n_qp, n_dofs, variant, ws, scatter, padding = k
            print(
                f"- {backend:6s} | {device:25s} | n_elem={n_elem:8d} n_qp={n_qp:2d} dofs={n_dofs:2d} "
                f"var={variant:3s} ws={ws:d} sc={scatter:d} pad={padding:d} : "
                f"GF={gmu:8.2f}±{gsd:6.2f}, GB/s={bmu:8.2f}±{bsd:6.2f}, AI={amu:6.3f}"
            )
    else:
        print("\n[Author Assembly-Like] no data")
