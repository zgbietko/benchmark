...
EXTENDED_PROFILES_PATH = ROOT / "configs" / "extended_benchmark_profiles.json"
...
def _load_extended_profiles() -> dict:
    if not EXTENDED_PROFILES_PATH.exists():
        return {}
    try:
        return json.loads(EXTENDED_PROFILES_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {}
...
def _normalize_extended_profile_key(
    *,
    ...
    if platform_profile != "auto" and platform_profile in available_profiles:
        return platform_profile

    if platform_profile in ("intel_arc", "intel_igpu") and "intel" in available_profiles:
        return "intel"
    ...
    if backend == "hip":
        return "amd"
    if backend == "opencl":
        if platform_profile in available_profiles:
            return platform_profile
        if arch == "amd":
            return "amd"
        if arch == "intel":
            if "intel_arc" in available_profiles:
                return "intel_arc"
            if "intel" in available_profiles:
                return "intel"
        if system_name == "Darwin":
            return "apple"
        if "intel_arc" in available_profiles:
            return "intel_arc"
        if "intel_igpu" in available_profiles:
            return "intel_igpu"
        if "intel" in available_profiles:
            return "intel"
    return platform_profile if platform_profile in available_profiles else ""


def _extended_args_for_script(
    *,
    mode: str,
    rel_script: str,
    backend: str,
    ...
    profiles = _load_extended_profiles()
    if not isinstance(profiles, dict) or not profiles:
        return []

    profile_key = _normalize_extended_profile_key(
    ...
    profile_block = profiles.get(profile_key, {}) if profile_key else {}
    backend_block = profile_block.get(backend, {}) if isinstance(profile_block, dict) else {}
    raw_args = backend_block.get(rel_script, []) if isinstance(backend_block, dict) else []
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="standard",
        help="standard = wspolna sciezka porownawcza; extended = bogatsze, architektur-specyficzne profile diagnostyczne.",
    )
    args = ap.parse_args()
    ...
    print(f"[INFO] GPU benchmark mode: {args.benchmark_mode}")
    ...
    def _run_backend(b: str) -> bool:
        ok_local = True
        def _run_script(rel_script: str) -> bool:
            extra_args = _extended_args_for_script(
            ...
            if extra_args:
                print(f"[INFO] Extended args for {Path(rel_script).name}: {' '.join(extra_args)}")
            return _run_py(rel_script, dev_arg + extra_args) == 0
        if b == "metal":
            ok_local = ok_local and _run_script("gpu/metal/benchmarks/run_metal_bandwidth.py")
            ok_local = ok_local and _run_script("gpu/metal/benchmarks/run_metal_pointer_latency.py")
            ok_local = ok_local and _run_script("gpu/metal/benchmarks/run_metal_compute_fma.py")
            ok_local = ok_local and _run_script("gpu/metal/benchmarks/run_metal_compute_fma_peak.py")
        elif b == "cuda":
            ok_local = ok_local and _run_script("gpu/cuda/benchmarks/run_cuda_bandwidth.py")
            ok_local = ok_local and _run_script("gpu/cuda/benchmarks/run_cuda_pointer_latency.py")
            ok_local = ok_local and _run_script("gpu/cuda/benchmarks/run_cuda_compute_fma.py")
            ok_local = ok_local and _run_script("gpu/cuda/benchmarks/run_cuda_compute_fma_peak.py")
        elif b == "hip":
            ok_local = ok_local and _run_script("gpu/hip/benchmarks/run_hip_bandwidth.py")
            ok_local = ok_local and _run_script("gpu/hip/benchmarks/run_hip_pointer_latency.py")
            ok_local = ok_local and _run_script("gpu/hip/benchmarks/run_hip_compute_fma.py")
            ok_local = ok_local and _run_script("gpu/hip/benchmarks/run_hip_compute_fma_peak.py")
        elif b == "opencl":
            ok_local = ok_local and _run_script("gpu/opencl/benchmarks/run_opencl_bandwidth.py")
            ok_local = ok_local and _run_script("gpu/opencl/benchmarks/run_opencl_pointer_latency.py")
            ok_local = ok_local and _run_script("gpu/opencl/benchmarks/run_opencl_compute_fma.py")
            ok_local = ok_local and _run_script("gpu/opencl/benchmarks/run_opencl_compute_fma_peak.py")
        return ok_local
