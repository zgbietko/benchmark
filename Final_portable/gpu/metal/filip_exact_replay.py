...
def option_dump_dir(root: Path, case_name: str, variant: str, option_index: int) -> Path:
    base = resolve_dump_root(root)
    case_specific = base / variant / f"opt_{int(option_index):03d}"
    if base.name == case_name and case_specific.exists():
        return case_specific
    return base / case_name / variant / f"opt_{int(option_index):03d}"
