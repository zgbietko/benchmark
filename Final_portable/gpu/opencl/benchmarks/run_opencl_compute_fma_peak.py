...
from gpu.opencl.opencl_backend import init_opencl, opencl_fma_peak, get_device_name
...
def run_fma_peak_bench(
    ...
    print("=== GPU FMA PEAK benchmark (OpenCL) ===")
        ...
        for n_elems in n_elements_options:
            ...
            for iters_inner in iters_inner_options:
                ...
                for run_idx in range(runs_per_config):
                    ...
                    elapsed_s = opencl_fma_peak(ocl, n=n_elems, iters_inner=iters_inner)
                ...
                if gflops_runs:
                    ...
                    gflops_sigma = stats.pstdev(gflops_runs) if len(gflops_runs) > 1 else 0.0
                else:
                    gflops_peak = gflops_mean = gflops_sigma = float("nan")

                if energy_runs:
                    e_mean = stats.mean(energy_runs)
                    e_sigma = stats.pstdev(energy_runs) if len(energy_runs) > 1 else 0.0
                else:
                    e_mean = e_sigma = float("nan")

                if power_runs:
                    p_mean = stats.mean(power_runs)
                    p_sigma = stats.pstdev(power_runs) if len(power_runs) > 1 else 0.0
                else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GPU FMA peak benchmark (OpenCL).")
    parser.add_argument("--device-index", type=int, default=0)
    parser.add_argument(
        "--n-elements",
        type=str,
        default="1048576,4194304,16777216",
        help="Lista n_elements (liczba elementów wektora), np. '1048576,4194304,16777216'.",
    )
    parser.add_argument(
        "--iters-inner",
        type=str,
        default="1000,5000,10000",
        help="Lista iters_inner (liczba iteracji pętli FMA), np. '1000,5000,10000'.",
    )
    parser.add_argument(
        "--runs-per-config",
        type=int,
