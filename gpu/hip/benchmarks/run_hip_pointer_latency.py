...
from gpu.hip.hip_backend import init_hip, get_device_info, hip_pointer_chase_latency
...
def run_pointer_latency(
    ...
    if not hasattr(ctx.lib, "gpu_hip_pointer_chase_latency"):
        print("[WARN] gpu_hip_pointer_chase_latency missing in HIP library; rebuild gpu/hip/lib and rerun.")
        return
    info = get_device_info(ctx)
    gpu_name = info.name

    print("=== GPU pointer-chasing latency benchmark (HIP) ===")
    ...
    with csv_path.open("a", newline="") as f:
        ...
        for kb in sizes_kb:
            ...
            for run_idx in range(runs_per_size):
                ...
                elapsed_s = hip_pointer_chase_latency(ctx, n=n, iters=iters_inner)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="GPU pointer-chasing latency benchmark (HIP).")
